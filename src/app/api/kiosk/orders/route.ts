...
export async function POST(request: Request) {
    ...
    return NextResponse.json(
      {
        success: false,
        message:
          "Forbidden. Kiosk credentials required to create kiosk orders.",
      },
      { status: 403, headers },
    );
  }
  ...
  try {
    ...
    const computedTotal =
      totalAmount ||
      items.reduce(
        (sum: number, it: any) => sum + it.price * (it.quantity || 1),
        0,
      );
    ...
    if (paymentMethod === "SPLIT_PAYMENT") {
      if (!paymentSplit || typeof paymentSplit !== "object") {
        return NextResponse.json(
          {
            success: false,
            message: "Payment breakdown is required for Split Payment.",
          },
          { status: 400, headers },
        );
      }
      ...
      if (Math.abs(splitSum - computedTotal) > 0.01) {
        return NextResponse.json(
          {
            success: false,
            message: `Split payment total (₹${splitSum}) must equal order total (₹${computedTotal}).`,
          },
          { status: 400, headers },
        );
      }
    ...
    const deviceLabel =
      staff.deviceLabel || `${storeCode} Store Shopping Tablet`;
      ...
      paymentMethod === "SPLIT_PAYMENT"
        ? [
            ...
            { method: "CARD", amount: parseFloat(paymentSplit.card) || 0 },
            {
              ...
              amount: parseFloat(paymentSplit.bankTransfer) || 0,
            },
          ].filter((c) => c.amount > 0)
    ...
    const subtotal = items.reduce(
      (acc: number, it: any) => acc + (it.price / 1.18) * (it.quantity || 1),
      0,
    );
    ...
    const kioskOrder = {
      ...
      customerName:
        customerName || (isB2B ? b2bDetails?.companyName : "Walk-in Customer"),
      customerPhone,
      isB2B: Boolean(isB2B),
      b2bDetails: isB2B
        ? {
