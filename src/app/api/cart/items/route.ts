    ...
    if (process.env.DATABASE_URL) {
      ...
      if (!product || !product.inStock) {
        return NextResponse.json(
          { success: false, message: 'Requested hardware product is currently out of stock or unavailable.' },
          { status: 422 }
        );
      }
      ...
      if (variantId) {
        ...
        if (!variant) {
          return NextResponse.json({ success: false, message: 'Invalid product variant selected.' }, { status: 400 });
        }
      ...
      if (availableStock > 0 && requestedQty > availableStock) {
        return NextResponse.json(
          { success: false, message: `Only ${availableStock} units available in stock.` },
          { status: 422 }
        );
      }
      ...
      if (existingItem) {
        ...
        if (availableStock > 0 && newQty > availableStock) {
          return NextResponse.json(
            { success: false, message: `Cannot add more. Stock limit of ${availableStock} reached.` },
            { status: 422 }
          );
        }
        ...
        return NextResponse.json({ success: true, message: 'Cart item quantity updated.', data: updatedItem });
      }
      ...
      return NextResponse.json({ success: true, message: 'Item added to cart.', data: newItem });
    }
