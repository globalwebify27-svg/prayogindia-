...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { storageKey, originalName, mimeType, size, context = 'SUPPORT_ATTACHMENT', ticketId } = body;

    if (!storageKey || typeof storageKey !== 'string') {
      return NextResponse.json({ success: false, message: 'Storage key is required.' }, { status: 400 });
    }

    // Security Check: Customer can only finalize objects in their own user directory
    if (!storageKey.includes(`/${user.id}/`)) {
      return NextResponse.json({ success: false, message: 'Forbidden storage key.' }, { status: 403 });
    }
    ...
    return NextResponse.json(
      { success: false, message: error.message || 'Failed to complete upload registration.' },
      { status: 500 }
    );
  }
}
