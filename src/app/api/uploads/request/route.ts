...
export async function POST(request: Request) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Forbidden. Customers can only upload support ticket attachments.",
        },
        { status: 403 },
      );
    }
    ...
    if (
      ...
      return NextResponse.json(
        { success: false, message: "Valid filename is required." },
        { status: 400 },
      );
    }
    ...
    if (
      ...
      !ALLOWED_ATTACHMENT_MIME_TYPES.includes(mimeType.toLowerCase())
    ) {
      return NextResponse.json(
        {
          success: false,
          message: `Unsupported file type "${mimeType}". Allowed types: JPEG, PNG, WEBP, PDF.`,
        },
        { status: 415 },
      );
    }
    ...
    if (fileSizeNum > MAX_ATTACHMENT_SIZE_BYTES) {
      return NextResponse.json(
        {
          success: false,
          message: `File size exceeds the 10MB limit (requested: ${(fileSizeNum / (1024 * 1024)).toFixed(1)}MB).`,
        },
        { status: 413 },
      );
    }
    ...
    return NextResponse.json(
      {
        success: false,
        message: error.message || "Failed to request upload authorization.",
      },
      { status: 500 },
    );
  }
