...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to fetch reward ledger", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!email || typeof points !== "number" || points === 0) {
      return NextResponse.json(
        { error: "Invalid parameters: email and non-zero points required" },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { error: "Failed to create points adjustment", details: error.message },
      { status: 500 },
    );
  }
}
