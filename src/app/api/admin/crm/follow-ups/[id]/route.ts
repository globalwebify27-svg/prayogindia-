...
export async function PATCH(
  ...
  try {
    ...
    if (process.env.DATABASE_URL) {
      ...
      if (!existing) {
        return NextResponse.json({ success: false, message: "Follow-up not found" }, { status: 404, headers });
      }
      ...
      if (status === "COMPLETED") {
        await db.b2BActivity.create({
          data: {
            ...
            description: completionNotes || `Marked completed by ${staff.name}.`,
    ...
    return NextResponse.json({ success: true, message: "Follow-up updated (Mock Mode)" }, { headers });
  } catch (error: any) {
