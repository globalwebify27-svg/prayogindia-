...
import { sanitizeAdminUser, AUTH_ADMIN_COOKIE_NAME, AdminSessionUser } from '@/lib/adminAuth';
...
export async function POST(request: Request) {
  ...
  if (!rateLimit.allowed) {
    return NextResponse.json(
      { success: false, message: `Too many admin login attempts. Please try again in ${rateLimit.resetInSeconds} seconds.` },
      { status: 429, headers }
    );
  }
  ...
  try {
    ...
    if (process.env.DATABASE_URL) {
      ...
      if (!dbUser || (dbUserRole !== 'ADMIN' && dbUserRole !== 'SUPER_ADMIN')) {
        return NextResponse.json(
          { success: false, message: 'Invalid admin credentials or account does not have admin privileges.' },
          { status: 401, headers }
        );
      }
