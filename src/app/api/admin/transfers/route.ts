...
import { getStockTransfers, transferStockBetweenStores } from '@/lib/inventoryEngine';
...
export async function POST(request: Request) {
  ...
  if (!staff || (staff.role !== 'SUPER_ADMIN' && staff.role !== 'REGIONAL_MANAGER')) {
    return NextResponse.json(
      { success: false, message: 'Forbidden. Only Super Admin or Regional Manager can initiate stock transfers.' },
      { status: 403, headers }
    );
  }
  ...
  try {
    ...
    if (!sourceStoreId || !destinationStoreId || !items || !Array.isArray(items) || items.length === 0) {
      return NextResponse.json(
        { success: false, message: 'Missing source, destination, or items array.' },
        { status: 400, headers }
      );
    }
    ...
    if (!STORES[src] || !STORES[dest]) {
      return NextResponse.json(
        { success: false, message: 'Invalid source or destination store identifier.' },
        { status: 400, headers }
      );
    }
    ...
    return NextResponse.json(
      { success: false, message: error.message || 'Failed to process stock transfer' },
      { status: 500, headers }
    );
  }
}
