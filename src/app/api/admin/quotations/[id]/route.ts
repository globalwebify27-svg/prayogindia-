    ...
    try {
      const quotation = await db.quotation.findUnique({
        ...
        include: {
          ...
          items: {
            include: {
              product: { select: { id: true, name: true, images: true, sku: true, mrp: true } },
            },
          },
          revisions: {
...
export async function PATCH(
  ...
  try {
    ...
    if (process.env.DATABASE_URL) {
      ...
      if (existing.status === "CONVERTED") {
        return NextResponse.json(
          { success: false, message: "Cannot edit a quotation that has already been converted to an order." },
          { status: 400, headers }
        );
      }
      ...
      if (typeof shippingCharge === "number") updateData.shippingCharge = shippingCharge;
      ...
      if (Array.isArray(items)) {
        let subtotal = 0;
        let totalDiscount = 0;
        const currentTaxRate = typeof taxRate === "number" ? taxRate : existing.taxRate;
        ...
        const taxAmount = Math.round(netTaxable * (currentTaxRate / 100) * 100) / 100;
        const shipCost = typeof shippingCharge === "number" ? shippingCharge : existing.shippingCharge;
      ...
      const changeNote = revisionReason || (status ? `Quotation updated to ${status}` : "Quotation terms or pricing updated");
        ...
        data: {
          ...
          revisions: {
            create: {
              ...
              snapshotJson: JSON.stringify({ ...updateData, status: updateData.status || existing.status }),
      ...
      await recordAuditLog({
        ...
        action: status && existing.status !== status ? `QUOTATION_STATUS_${status}` : "QUOTATION_UPDATE",
    ...
    return NextResponse.json({ success: true, message: "Quotation updated (Mock Mode)" }, { headers });
  } catch (error: any) {
    return NextResponse.json({ success: false, message: error.message }, { status: 500, headers });
  }
}
