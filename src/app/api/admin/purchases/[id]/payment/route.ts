...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!amount || amount <= 0) {
      return NextResponse.json({ success: false, message: "Valid payment amount is required" }, { status: 400, headers });
    }

    if (!paymentMethod) {
      return NextResponse.json({ success: false, message: "Payment method is required" }, { status: 400, headers });
    }

    if (!process.env.DATABASE_URL) {
      return NextResponse.json({ success: true, message: "Payment recorded (mock)" }, { headers });
    }
    ...
    const newPaymentStatus = newAmountDue <= 0.01 ? "PAID" : newAmountPaid > 0 ? "PARTIALLY_PAID" : "UNPAID";

    await db.purchaseOrder.update({
      where: { id: purchaseOrderId },
      data: { amountPaid: newAmountPaid, amountDue: newAmountDue, paymentStatus: newPaymentStatus },
    });
    ...
    await db.supplier.update({
      ...
      data: {
        ...
        totalPurchases: po.amountPaid === 0 ? { increment: po.totalAmount } : undefined,
      },
    });
