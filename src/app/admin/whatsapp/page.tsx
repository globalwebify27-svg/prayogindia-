...
const INITIAL_TEMPLATES: WhatsAppTemplate[] = [
  {
    ...
    content: 'Hi {{customer_name}}, thank you for your order #{{order_number}} on Prayog India! Your invoice has been generated. Track live delivery: {{tracking_url}}',
  ...
  },
  {
    ...
    content: 'Hello {{customer_name}}, your consignment #{{order_number}} is on the way via {{courier_name}} (AWB: {{awb_number}}). Expected arrival: {{eta_date}}.',
    status: 'Approved (Meta API)',
    variables: ['customer_name', 'order_number', 'courier_name', 'awb_number', 'eta_date'],
  },
  {
    id: 'tpl-3',
    name: 'out_of_stock_availability_reply',
    triggerEvent: 'Customer Clicked "Ask Availability on WhatsApp"',
    content: 'Hi {{customer_name}}, regarding your inquiry for {{product_name}} (SKU: {{sku}}): New stock has arrived at our Ranchi hub! You can order now: {{product_url}}',
    status: 'Approved (Meta API)',
  ...
  },
  {
    ...
    content: 'Thank you for visiting Prayog India {{store_name}} Store! Your digital GST Tax Invoice #{{invoice_number}} (Total ₹{{total_amount}}) is available here: {{invoice_pdf_url}}',
    status: 'Approved (Meta API)',
    variables: ['store_name', 'invoice_number', 'total_amount', 'invoice_pdf_url'],
  },
];
...
const INITIAL_ENQUIRIES: WhatsAppEnquiry[] = [
  {
    ...
    status: 'Replied',
    messageText: 'Hi Prayog India, I am interested in Pixhawk 6C (SKU: PRG-UAV-601). Please let me know the availability and latest B2B price.',
  },
  {
    id: 'enq-102',
    ...
    messageText: 'Hi Prayog India, I need 20 units of Raspberry Pi 5 8GB for university lab setup. Please share quotation.',
  },
  {
    id: 'enq-103',
    ...
    status: 'New Enquiry',
    messageText: 'Hi Prayog India, is this LiPo battery safe to ship to Hyderabad? Need for college drone competition.',
  },
];

export default function AdminWhatsAppPage() {
  const [templates, setTemplates] = useState<WhatsAppTemplate[]>(INITIAL_TEMPLATES);
  const [enquiries, setEnquiries] = useState<WhatsAppEnquiry[]>(INITIAL_ENQUIRIES);
  const [searchQuery, setSearchQuery] = useState('');
  const [activeTab, setActiveTab] = useState<'enquiries' | 'templates' | 'settings'>('enquiries');

  // WhatsApp API Settings State
  const [whatsappPhone, setWhatsappPhone] = useState('+91 98765 43210');
  const [businessAccountId, setBusinessAccountId] = useState('WABA_PRAG_IND_2026');
  const [apiKey, setApiKey] = useState('EAAGk18992xza8991288aa091...');

  const handleUpdateStatus = (id: string, newStatus: WhatsAppEnquiry['status']) => {
    ...
    <div className="p-6 sm:p-8 space-y-8 animate-in fade-in duration-300">
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-emerald-700 bg-emerald-50 px-3.5 py-1 rounded-full border border-emerald-200 flex items-center gap-1">
              <MessageSquare className="w-3 h-3 text-emerald-600" /> Section 74 · WhatsApp Business Platform Hub
            </span>
          </div>
          ...
          <p className="text-xs text-slate-500">
            Manage Meta Business Platform message templates for order notifications, dispatch AWB alerts, out-of-stock product inquiries, and digital POS invoices.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-2">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'enquiries' ? 'bg-emerald-600 text-white shadow-xs' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'templates' ? 'bg-emerald-600 text-white shadow-xs' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'settings' ? 'bg-emerald-600 text-white shadow-xs' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-2xs space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-500">Customer conversations initiated via "Ask Availability on WhatsApp" with pre-filled SKU attributes.</p>
            </div>
            <span className="text-xs font-bold text-slate-400">
              {enquiries.filter(e => e.status === 'New Enquiry').length} Action Required
            </span>
          </div>
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {enquiries.map((enq) => (
                  <tr key={enq.id} className="hover:bg-slate-50/80 transition-colors">
                    <td className="py-3.5">
                      <div className="font-extrabold text-slate-900">{enq.customerName}</div>
                      <div className="font-mono text-[10px] text-emerald-700 font-bold">{enq.mobileNumber}</div>
                    </td>

                    <td className="py-3.5">
                      <div className="font-bold text-slate-900">{enq.productName}</div>
                      <span className="font-mono text-[10px] text-slate-400 font-bold">{enq.sku}</span>
                    </td>

                    <td className="py-3.5 text-slate-600 max-w-xs truncate" title={enq.messageText}>
                      {enq.messageText}
                    </td>
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-2xs space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-500">Configured HSM templates for automated backend transactional dispatches.</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {templates.map((tpl) => (
              <div key={tpl.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-mono font-black text-slate-900 text-xs">{tpl.name}</span>
                ...
                <div className="text-[10px] font-bold text-[#00AEEF] uppercase">{tpl.triggerEvent}</div>
                <p className="text-xs text-slate-700 bg-white p-3 rounded-xl border border-slate-200/80 font-medium leading-relaxed">
                  {tpl.content}
                </p>
                <div className="flex flex-wrap gap-1 pt-1">
                  {tpl.variables.map(v => (
                    <span key={v} className="bg-slate-200 text-slate-700 font-mono text-[9px] px-1.5 py-0.5 rounded">
                      {`{{${v}}}`}
                    </span>
                  ))}
                </div>
      ...
      {activeTab === 'settings' && (
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-2xs space-y-4 max-w-xl">
          ...
          <form onSubmit={(e) => { e.preventDefault(); alert('WhatsApp API settings saved!'); }} className="space-y-4 text-xs">
            <div>
            ...
            <div>
              <label className="block font-bold text-slate-700 mb-1">Permanent Meta Access Token</label>
              <input
                type="password"
                value={apiKey}
