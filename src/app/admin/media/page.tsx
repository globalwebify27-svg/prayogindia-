...
const INITIAL_ASSETS: MediaAsset[] = [
  {
    ...
    altText: 'Arduino Uno R4 WiFi with LED Matrix and Renesas RA4M1 Microcontroller',
  ...
  },
  {
    ...
    altText: 'Raspberry Pi 5 8GB RAM Single Board Computer with BCM2712 Quad Core',
  ...
  },
  {
    ...
    altText: 'Holybro Pixhawk 6C Autopilot Flight Controller for Quadcopter Drones',
  ...
  },
  {
    ...
    altText: 'NVIDIA Jetson Orin Nano 8GB Developer Kit for Edge AI Computer Vision',
  ...
  },
  {
    ...
    altText: 'Prayog India Atal Tinkering Lab Setup Hardware Benches with Microcontroller Kits',
  ...
  },
  {
    ...
    altText: 'LiPo Battery Safe Charging, Surface Freight Transportation & Storage Protocol PDF',
  ...
  },
  {
    ...
    altText: 'STMicroelectronics STM32F401RE Pinout Architecture & Schematic Datasheet',
  ...
  },
  {
    ...
    altText: '3D CAD Printable STL Chassis for 4WD Omni-Directional Mecanum Robot',
  ...
  },
  {
    ...
    sizeKb: 340,
    folder: 'Services',
    altText: '6-Axis Industrial Robotics Manipulator Lab Installation by Prayog India Engineers',
...
export default function MediaLibraryPage() {
  ...
  const [newFileType, setNewFileType] = useState<'image' | 'pdf' | 'cad' | 'video'>('image');
  const [newFolder, setNewFolder] = useState<'Products' | 'Banners' | 'Services' | 'Datasheets' | '3D Models'>('Products');
  ...
  const filteredAssets = assets.filter((item) => {
    ...
    const matchesSearch = 
      ...
      (item.associatedSku && item.associatedSku.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const handleUploadSubmit = (e: React.FormEvent) => {
    ...
    const newAsset: MediaAsset = {
      ...
      url: newFileUrl.trim(),
      type: newFileType,
      mimeType: newFileType === 'image' ? 'image/webp' : newFileType === 'pdf' ? 'application/pdf' : newFileType === 'cad' ? 'model/stl' : 'video/mp4',
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-[#0F172A] text-white p-6 sm:p-8 rounded-3xl border border-slate-800 shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400 font-bold">Cloudinary / AWS S3 Production Storage</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-black text-white mt-1">Media Library Management</h1>
          <p className="text-xs text-slate-400 max-w-2xl font-medium">
            Centralized repository for hardware photography, component schematics, PDF datasheets, and 3D CAD models with direct SKU attribution and CDN links.
          </p>
        </div>
      ...
      {uploadSuccessAlert && (
        <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold p-3.5 rounded-2xl flex items-center gap-2 animate-in fade-in">
          ...
          <span>New media asset uploaded and published to Prayog CDN cache!</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-1">
          ...
          <div className="text-2xl font-black text-slate-900">{assets.length} Files</div>
          <div className="text-[11px] text-slate-500 font-medium">Across all hardware categories</div>
        </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-1">
          ...
          <div className="text-2xl font-black text-slate-900">{totalStorageMb} MB</div>
          <div className="text-[11px] text-slate-500 font-medium">of 5.0 GB CDN Quota (8.5%)</div>
        </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-1">
          ...
          <div className="text-2xl font-black text-emerald-700">
            {assets.filter(a => a.folder === 'Products' || a.type === 'image').length}
          </div>
          <div className="text-[11px] text-slate-500 font-medium">Optimized WebP Format</div>
        </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-1">
          ...
          <div className="text-[11px] text-slate-500 font-medium">PDFs, STLs &amp; STEP models</div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-4 shadow-xs space-y-3">
        <div className="flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1 overflow-x-auto w-full md:w-auto pb-1 md:pb-0 text-xs font-bold">
            {['ALL', 'image', 'pdf', 'cad'].map((t) => (
              ...
              >
                {t === 'ALL' ? 'All Formats' : t === 'image' ? 'Photos' : t === 'pdf' ? 'Datasheets' : '3D CAD'}
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200 shrink-0">
            <button
              ...
              className={`p-1.5 rounded-lg transition-colors cursor-pointer ${
                viewMode === 'grid' ? 'bg-white shadow-xs text-[#00AEEF]' : 'text-slate-500 hover:text-slate-900'
              }`}
              title="Grid View"
            >
            ...
            <button
              ...
              className={`p-1.5 rounded-lg transition-colors cursor-pointer ${
                viewMode === 'list' ? 'bg-white shadow-xs text-[#00AEEF]' : 'text-slate-500 hover:text-slate-900'
              }`}
              title="List View"
            >
        ...
        <div className="flex items-center gap-2 overflow-x-auto pt-2 border-t border-slate-100 text-xs">
          <span className="text-[10px] font-extrabold uppercase text-slate-400 shrink-0">Folder:</span>
          {['ALL', 'Products', 'Banners', 'Services', 'Datasheets', '3D Models'].map((folder) => (
            <button
              key={folder}
            ...
            >
              {folder} ({folder === 'ALL' ? assets.length : assets.filter(a => a.folder === folder).length})
            </button>
          ))}
        </div>
      </div>
          ...
          {filteredAssets.map((asset) => (
            ...
            >
              ...
              >
                {asset.type === 'image' ? (
                  ...
                  <div className="flex flex-col items-center gap-2 text-blue-700 p-4 text-center">
                    ...
                    <span className="text-xs font-mono font-bold">3D STL CAD Model</span>
                  </div>
                ) : (
                  <div className="flex flex-col items-center gap-2 text-purple-700 p-4 text-center">
                    <Video className="w-12 h-12" />
                    <span className="text-xs font-mono font-bold">Video File</span>
                  </div>
                )}
              ...
              <div className="p-3.5 space-y-2 text-xs">
                <div>
                  <h4 className="font-bold text-slate-900 truncate" title={asset.name}>
                    {asset.name}
                  </h4>
                  <div className="flex items-center justify-between text-[10px] text-slate-400 font-mono mt-0.5">
                    <span>{asset.sizeKb} KB • {asset.mimeType}</span>
                    ...
                    <td className="p-3.5 pl-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-slate-400 line-clamp-1">{asset.altText}</div>
                        </div>
                      </div>
                    </td>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 w-full max-w-2xl overflow-hidden shadow-2xl space-y-0 my-8">
            ...
            <div className="p-5 bg-slate-900 text-white flex items-center justify-between">
              <div>
                ...
                <h3 className="text-base font-black text-white truncate max-w-md">{activeAsset.name}</h3>
              </div>
              <button 
            ...
            <div className="p-6 space-y-5 text-xs">
              ...
              <div className="h-64 bg-slate-100 rounded-2xl border border-slate-200 overflow-hidden flex items-center justify-center">
                {activeAsset.type === 'image' ? (
                  <img src={activeAsset.url} alt={activeAsset.altText} className="w-full h-full object-contain" />
                ) : activeAsset.type === 'pdf' ? (
                  <div className="flex flex-col items-center gap-3 text-amber-700">
                    <FileText className="w-16 h-16" />
                    <span className="font-mono font-bold">PDF Document Ready</span>
                    <a
                  ...
                  <div className="flex flex-col items-center gap-3 text-blue-700">
                    ...
                    <span className="font-mono font-bold">3D STL / STEP Model Asset</span>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-200">
                <div>
                  ...
                  <div className="font-bold text-slate-900">{activeAsset.folder}</div>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Associated SKU</span>
                  <div className="font-mono font-bold text-[#00AEEF]">{activeAsset.associatedSku || 'None'}</div>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase">File Size</span>
                  <div className="font-mono font-bold text-slate-900">{activeAsset.sizeKb} KB</div>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase">MIME Type</span>
                  <div className="font-mono text-slate-700">{activeAsset.mimeType}</div>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Dimensions</span>
                  <div className="text-slate-700">{activeAsset.dimensions || 'N/A'}</div>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Uploaded On</span>
              ...
              {/* CDN URL Direct Copy */}
              <div>
                <label className="block font-bold text-slate-700 uppercase mb-1">Public CDN URL</label>
              ...
              {/* Alt Text for SEO */}
              <div>
                <label className="block font-bold text-slate-700 uppercase mb-1">SEO Image Alt Text</label>
                <input
                  type="text"
                  ...
                  onChange={(e) => {
                    ...
                    setAssets(assets.map(a => a.id === activeAsset.id ? updated : a));
                  }}
          ...
          <div className="bg-white rounded-3xl border border-slate-200 w-full max-w-xl overflow-hidden shadow-2xl space-y-0 my-8">
            ...
            <div className="p-5 bg-slate-900 text-white flex items-center justify-between">
              <div>
                ...
                <h3 className="text-base font-black text-white">Add New Hardware Asset</h3>
              </div>
              <button 
            ...
            <form onSubmit={handleUploadSubmit} className="p-6 space-y-4 text-xs">
              <div>
                <label className="block font-bold text-slate-700 uppercase mb-1">Asset Name / Filename</label>
                <input
                  type="text"
                  required
                  value={newFileName}
              ...
              <div>
                <label className="block font-bold text-slate-700 uppercase mb-1">Direct CDN URL / Source Link</label>
                <input
                  type="url"
                  required
                  value={newFileUrl}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-slate-700 uppercase mb-1">Format Type</label>
                  <select
                    value={newFileType}
                ...
                <div>
                  <label className="block font-bold text-slate-700 uppercase mb-1">Folder Category</label>
                  <select
                    value={newFolder}
              ...
              <div>
                <label className="block font-bold text-slate-700 uppercase mb-1">Associated Hardware SKU (Optional)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block font-bold text-slate-700 uppercase mb-1">SEO Alt Text / Description</label>
                <input
                  type="text"
                  value={newAltText}
