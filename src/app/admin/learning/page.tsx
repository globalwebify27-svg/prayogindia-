  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-0.5">
            Create and manage educational articles, tutorials, and guides for STEM kits, drones, and robotics
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Total Articles", value: items.length, color: "bg-slate-50 border-slate-200 text-slate-900" },
          { label: "Beginner", value: items.filter((i) => i.level === "Beginner").length, color: "bg-emerald-50 border-emerald-200 text-emerald-800" },
          { label: "Intermediate", value: items.filter((i) => i.level === "Intermediate").length, color: "bg-amber-50 border-amber-200 text-amber-800" },
          { label: "Advanced", value: items.filter((i) => i.level === "Advanced").length, color: "bg-red-50 border-red-200 text-red-800" },
        ].map((stat) => (
          <div key={stat.label} className={`rounded-2xl border p-4 ${stat.color}`}>
            <span className="text-[10px] font-extrabold uppercase text-slate-500 tracking-wider block">{stat.label}</span>
              ...
              {loading ? (
                ...
                filtered.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="p-4">
                      <span className={`px-2 py-0.5 rounded-lg text-[11px] font-bold border ${LEVEL_COLORS[item.level] || LEVEL_COLORS.Beginner}`}>
                        {item.level}
                      </span>
                    </td>
                    <td className="p-4">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div onClick={() => setShowModal(false)} className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs animate-in fade-in" />
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="text-[10px] font-black uppercase text-slate-500 block mb-1">Title *</label>
                  <input
                    required
                    type="text"
                    value={fTitle}
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-500 block mb-1">Slug *</label>
                  <input
                    required
                    type="text"
                    value={fSlug}
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-500 block mb-1">Read Time</label>
                  <input
                    type="text"
                    value={fReadTime}
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-500 block mb-1">Level</label>
                  <select
                    value={fLevel}
                ...
                <div className="col-span-2">
                  <label className="text-[10px] font-black uppercase text-slate-500 block mb-1">Banner Image URL</label>
                  <input
                    type="url"
                    value={fBanner}
                ...
                <div className="col-span-2">
                  <label className="text-[10px] font-black uppercase text-slate-500 block mb-1">Content (Markdown) *</label>
                  <textarea
                    required
                    rows={8}
                    value={fContent}
