...
export default function AdminProfitPage() {
  const [profitOrders, setProfitOrders] =
    useState<OrderProfitRecord[]>(MOCK_PROFIT_ORDERS);
  ...
  const [profitRules, setProfitRules] = useState<ProfitSharingRule[]>([
    {
      role: "Intern / Trainee",
      percentage: 5,
      applicableScope: "5% of Net Order Profit",
    },
    {
      role: "Sales Executive",
      percentage: 7,
      applicableScope: "7% of Net Order Profit",
    },
    {
      role: "Store Manager",
      percentage: 2,
      applicableScope: "2% of Total Store Net Profit",
    },
  ]);

  const filteredOrders = profitOrders.filter((ord) => {
    const matchesStore =
      filterStore === "All" || ord.storeLocation.includes(filterStore);
    const matchesChannel =
      filterChannel === "All" || ord.customerType === filterChannel;
    const matchesExec =
      filterExecutive === "All" || ord.salesExecutive === filterExecutive;
  ...
  const totalPurchaseCost = filteredOrders.reduce(
    (sum, o) => sum + o.purchaseCost,
    0,
  );
  const totalShippingCost = filteredOrders.reduce(
    (sum, o) => sum + o.shippingCost,
    0,
  );
  const totalNetProfit = filteredOrders.reduce(
    (sum, o) => sum + o.netProfit,
    0,
  );
  const averageMargin =
    ...
    <div className="p-6 sm:p-8 space-y-8 animate-in fade-in duration-300">
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-[#00AEEF] bg-[#E0F7FC] px-3.5 py-1 rounded-full border border-[#00AEEF]/20 flex items-center gap-1">
              <Lock className="w-3 h-3" /> Section 69–72 · Permission-Restricted
              Profit Dashboard
            </span>
          </div>
          ...
          <p className="text-xs text-slate-500">
            Visible only to Super Admin &amp; Store Managers. Net Profit =
            Selling Price − Purchase Cost − Shipping − Additional Charges.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-2xs space-y-1">
          ...
          <span className="text-[10px] text-slate-400 font-bold">
            + ₹{totalShippingCost.toLocaleString()} freight expenses
          </span>
        </div>
        ...
        <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-2xs space-y-1">
          ...
          <span className="text-[10px] text-purple-700 font-bold">
            ₹25,000 Profit Generated (7% Pool)
          </span>
        </div>
      </div>
              ...
              {filteredOrders.map((ord) => (
                ...
                >
                  ...
                  <td className="py-3.5 text-right font-mono text-slate-400 text-[11px]">
                    ₹
                    {(
                      ord.shippingCost + ord.additionalCharges
                    ).toLocaleString()}
                  </td>
        ...
        <div className="fixed inset-0 z-50 overflow-hidden flex items-center justify-center p-4">
          ...
          <div className="relative max-w-lg w-full bg-white rounded-3xl p-6 sm:p-8 shadow-2xl z-10 space-y-4 text-xs animate-in zoom-in-95">
            ...
            <p className="text-slate-500">
              Define commission % calculated directly from eligible Net Order
              Profits ($Selling - Purchase - Shipping - Overhead$).
            </p>
            ...
            <div className="space-y-3">
              {profitRules.map((rule, idx) => (
                <div
                  key={idx}
                  className="bg-slate-50 border border-slate-200 p-3 rounded-2xl flex items-center justify-between gap-3"
                >
                  <div>
                          ...
                          prev.map((r, i) =>
                            i === idx
                              ? {
                                  ...r,
                                  percentage: val,
                                  applicableScope: `${val}% of Net Profit`,
                                }
                              : r,
                          ),
                        );
            ...
            <div className="bg-purple-50 border border-purple-200 p-3 rounded-2xl text-[11px] text-purple-900 font-medium">
              🔒 Immutable Audit Rule: Historical reports retain the calculation
              rate active at the time the order was closed.
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
              ...
              <button
                onClick={() => {
                  ...
                  alert(
                    "Profit sharing rules updated & saved to system policy.",
                  );
                }}
