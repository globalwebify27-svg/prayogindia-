...
function SuperAdminLoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Ensure only SUPER_ADMIN role can enter via /admin
      if (data.user?.role !== "SUPER_ADMIN") {
        if (data.user?.role === "STORE_MANAGER" && data.user?.storeCode) {
          setError(
            `Store Managers must login at their branch portal: domain/${data.user.storeCode.toLowerCase()}/storemanager`,
          );
        } else if (data.user?.role === "KIOSK_USER" && data.user?.storeCode) {
          setError(
            `Kiosk devices must activate at: domain/${data.user.storeCode.toLowerCase()}/device`,
          );
        } else {
      ...
      const destination = redirectTarget || "/admin/dashboard";
      router.push(destination);
    } catch {
      setError("Network error while authenticating administrator.");
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#071920] flex items-center justify-center p-4 sm:p-8 font-sans selection:bg-[#F26522] selection:text-white relative overflow-hidden">
      ...
      <div className="w-full max-w-5xl relative z-10 py-8">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 items-center">
          ...
          {/* Right Column: Welcome & Login Form */}
          <div className="md:col-span-6 flex flex-col items-center md:items-start w-full max-w-md mx-auto md:mx-0">
            <div className="w-full text-center md:text-left mb-6 sm:mb-8">
              ...
              <p className="text-[11px] sm:text-xs font-bold uppercase tracking-[0.22em] text-slate-300 mt-2.5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-sm w-full p-6 space-y-4 shadow-2xl text-center">
            ...
            <p className="text-xs text-slate-300 leading-relaxed">
              Super Administrator recovery is handled securely via root database
              console or system hosting operations.
            </p>
            <button
