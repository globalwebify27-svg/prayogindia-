...
export default function KioskCatalogPage() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'CASH' | 'UPI' | 'CARD' | 'BANK_TRANSFER' | 'SPLIT_PAYMENT'>('CASH');
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    if (requireB2B && (!companyName.trim() || !gstin.trim() || !companyAddress.trim())) {
      alert('Please fill all required B2B Invoice fields (Company Name, GST Number, Company Address).');
      return;
    }

    if (paymentMethod === 'SPLIT_PAYMENT') {
      if (Math.abs(currentSplitSum - totalAmount) > 0.01) {
        alert(`Split breakdown (₹${currentSplitSum}) must exactly equal the total payable amount (₹${totalAmount}). Difference: ₹${Math.abs(totalAmount - currentSplitSum)}`);
        return;
      }
    }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {loading ? (
          <div className="col-span-full py-20 text-center text-slate-500">
            ...
            <p className="text-xs font-bold uppercase tracking-wider text-slate-600">Fetching Store Inventory...</p>
          </div>
        ) : products.length === 0 ? (
          <div className="col-span-full py-16 text-center text-slate-500 bg-white border border-slate-200 rounded-3xl p-8 shadow-xs">
            <p className="text-sm font-bold text-slate-700">No products match your search in this store branch.</p>
            <p className="text-xs text-slate-400 mt-1">Try searching by SKU, component name, or selecting a different category.</p>
          </div>
        ) : (
          products.map(prod => (
            <div
            ...
            <div>
              <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-base font-black text-slate-900">Store Walk-in Basket</h2>
                </div>
                <button
                    ...
                    <div key={item.id} className="py-3.5 flex items-center justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[11px] text-slate-500 font-mono mt-0.5">
                          ₹{item.price} × {item.quantity} = <span className="text-slate-900 font-bold">₹{item.price * item.quantity}</span>
                        </div>
                      </div>
          ...
          <div className="w-full max-w-md bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 space-y-6 shadow-2xl">
            {orderComplete ? (
              <div className="text-center space-y-5 py-4 max-h-[85vh] overflow-y-auto pr-1">
                ...
                <div>
                  <h2 className="text-xl font-black text-slate-900">Order Sent to Store Counter!</h2>
                  <p className="text-xs text-slate-500 mt-1">
                    Order Reference: <span className="font-mono font-bold text-[#00AEEF]">{orderComplete.orderNumber}</span>
                  </p>
                </div>
                ...
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 text-xs font-mono text-left space-y-3">
                  <div className="flex justify-between border-b border-slate-200 pb-2">
                    ...
                    <span className="font-bold text-slate-900">{orderComplete.store || orderComplete.storeCode} Hub</span>
                  </div>

                  <div className="flex justify-between border-b border-slate-200 pb-2">
                    <span className="text-slate-500">Customer:</span>
                    <span className="font-bold text-slate-900">{orderComplete.customerName}</span>
                  </div>

                  <div className="flex justify-between border-b border-slate-200 pb-2">
                    <span className="text-slate-500">Payment Mode:</span>
                    <span className="text-emerald-700 font-bold">{orderComplete.paymentMethod}</span>
                  </div>

                  <div className="flex justify-between items-center pt-1 text-sm">
                    <span className="font-bold text-slate-700">Total Payable:</span>
                    <span className="text-base font-black text-slate-900 font-mono">₹{orderComplete.totalAmount?.toLocaleString('en-IN')}</span>
                  </div>
                </div>
                ...
                <div className="bg-blue-50 border border-blue-200 p-4 rounded-2xl text-xs text-blue-900 text-center space-y-1">
                  <div className="font-bold text-slate-900 flex items-center justify-center gap-1.5">
                    <Sparkles className="w-4 h-4 text-[#00AEEF]" /> Please Proceed to the Billing Counter
                  </div>
                  <p className="text-[11px] text-slate-600">
                    Our Store Manager will print or WhatsApp your official Tax Invoice & hand over your items.
                  </p>
                </div>
              ...
              <>
                <div className="flex items-center justify-between">
                  <h2 className="text-base font-black text-slate-900">Walk-in Customer Checkout</h2>
                  <button
                ...
                <form onSubmit={handlePlaceOrder} className="space-y-4 max-h-[75vh] overflow-y-auto pr-1">
                    ...
                    <div className="bg-amber-50/50 p-4 rounded-2xl border border-amber-200 space-y-3 animate-fade-in">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="font-mono text-slate-600">
                          Target: <span className="text-slate-900 font-bold">₹{totalAmount.toLocaleString('en-IN')}</span>
                        </span>
                      </div>

                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div>
                          <label className="block text-[11px] text-slate-600 mb-1">Cash Amount (₹)</label>
                          <input
                            type="number"
                            min="0"
                        ...
                        <div>
                          <label className="block text-[11px] text-slate-600 mb-1">UPI Amount (₹)</label>
                          <input
                            type="number"
                            min="0"
                        ...
                        <div>
                          <label className="block text-[11px] text-slate-600 mb-1">Card Amount (₹)</label>
                          <input
                            type="number"
                            min="0"
                        ...
                        <div>
                          <label className="block text-[11px] text-slate-600 mb-1">Bank Transfer (₹)</label>
                          <input
                            type="number"
                            min="0"
                            value={splitAmounts.bankTransfer}
                            onChange={(e) => setSplitAmounts({ ...splitAmounts, bankTransfer: e.target.value })}
                            placeholder="0"
                            className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-mono text-slate-900 focus:outline-none focus:border-amber-400"
                          />
                        </div>
                      ...
                      <div className="pt-2 border-t border-amber-200/80 flex justify-between items-center text-xs font-mono">
                        ...
                        <span className={Math.abs(currentSplitSum - totalAmount) < 0.01 ? 'text-emerald-600 font-bold' : 'text-amber-700 font-bold'}>
                          ₹{currentSplitSum} / ₹{totalAmount}
                          {Math.abs(currentSplitSum - totalAmount) >= 0.01 && (
                            <span className="text-[10px] ml-1 text-red-600">
                              ({currentSplitSum > totalAmount ? `+₹${currentSplitSum - totalAmount} over` : `-₹${totalAmount - currentSplitSum} remaining`})
                            </span>
                          )}
                        </span>
                      </div>
                  ...
                  <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 flex justify-between items-center text-xs font-mono">
                    <span className="text-slate-600">Grand Total ({totalCount} items):</span>
                    <span className="text-sm font-bold text-slate-900">₹{totalAmount.toLocaleString('en-IN')}</span>
                  </div>

                  <button
                    type="submit"
                    disabled={submittingOrder || (paymentMethod === 'SPLIT_PAYMENT' && Math.abs(currentSplitSum - totalAmount) >= 0.01)}
                  ...
                  >
                    {submittingOrder ? 'Generating Order...' : `Confirm & Pay ₹${totalAmount}`}
                  </button>
                </form>
              </>
            )}
          ...
          <div className="w-full max-w-xl bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 space-y-6 shadow-2xl relative">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-base font-black text-slate-900 line-clamp-1">{view360Product.name}</h3>
                  <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                    Interactive 360° Inspection Stage • {view360Product.category}
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <p className="text-center text-[11px] text-slate-400">
                Drag the slider or use 45° step buttons to inspect component pins, sockets, & PCB traces.
              </p>
            </div>
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-4">
              <div>
                ...
                {view360Product.mrp && view360Product.mrp > view360Product.price && (
