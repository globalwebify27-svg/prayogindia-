...
function BranchStoreManagerLoginForm() {
  ...
  const store = STORES[branchParam] || {
    id: branchParam,
    name: `${branchParam.toUpperCase()} Branch Store`,
    city: branchParam.toUpperCase(),
  };

  const [username, setUsername] = useState(`${branchParam}_manager`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        setError(
          `Access Denied. Your account is assigned to ${data.user?.storeCode || "another store"}, not ${branchParam.toUpperCase()}.`,
        );
        setLoading(false);
        return;
      }
    ...
    <div className="min-h-screen bg-[#071920] flex items-center justify-center p-4 sm:p-8 font-sans selection:bg-[#F26522] selection:text-white relative overflow-hidden">
      ...
      <div className="w-full max-w-5xl relative z-10 py-8">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 items-center">
          {/* Left Column: Brand & Branch Identity */}
          <div className="md:col-span-5 flex flex-col items-center justify-center text-center space-y-4 py-6 md:py-12">
            ...
            <div className="inline-flex items-center gap-2 bg-slate-900/90 border border-slate-700/80 px-4 py-1.5 rounded-full text-xs font-bold text-[#00AEEF] mt-4 shadow-md">
              <Store className="w-3.5 h-3.5 text-[#FFC20E]" />
              <span className="uppercase tracking-wider">{store.name}</span>
            </div>
          </div>
          ...
          <div className="md:col-span-6 flex flex-col items-center md:items-start w-full max-w-md mx-auto md:mx-0">
            <div className="w-full text-center md:text-left mb-6 sm:mb-8">
              ...
              <p className="text-[11px] sm:text-xs font-bold uppercase tracking-[0.2em] text-slate-300 mt-2.5">
                PLEASE LOGIN TO {branchParam.toUpperCase()} STORE MANAGER
                DASHBOARD.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="w-full space-y-4">
              ...
              <div className="pt-2">
                ...
                >
                  {loading ? (
                    ...
                    <span>LOGIN AS {branchParam.toUpperCase()} MANAGER</span>
                  )}
                </button>
              </div>
              ...
              <div className="text-center pt-2">
                <button
                  ...
                  className="text-[10px] sm:text-[11px] font-bold tracking-[0.16em] text-slate-300 hover:text-white uppercase transition-colors cursor-pointer"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-sm w-full p-6 space-y-4 shadow-2xl text-center">
            ...
            <p className="text-xs text-slate-300 leading-relaxed">
              Store Manager passwords can be reset by the{" "}
              <strong>Super Administrator</strong> in Central HQ or by
              contacting{" "}
              <span className="text-[#00AEEF] font-mono">
                admin@prayogindia.com
              </span>
              .
            </p>
            <button
              onClick={() => setShowForgotModal(false)}
              className="w-full bg-[#F26522] hover:bg-[#D94F12] text-white text-xs font-bold py-2.5 rounded-lg transition-colors cursor-pointer uppercase tracking-wider"
            >
              Close
            </button>
          </div>
...
export default function BranchStoreManagerPage() {
  return (
    <Suspense
      fallback={
