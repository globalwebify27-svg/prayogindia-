...
export default function Home() {
  ...
  const getCategorySlug = (catName: string): string => {
    ...
    if (lower.includes('drone') || lower.includes('uav')) return '/categories/drone-technology';
    if (lower.includes('robot')) return '/categories/robotics';
    if (lower.includes('arduino') || lower.includes('devboard') || lower.includes('development') || lower.includes('single board')) return '/categories/arduino-development-boards';
    if (lower.includes('stem') || lower.includes('education')) return '/categories/stem-kits';
    if (lower.includes('sensor')) return '/categories/sensors-modules';
    if (lower.includes('iot') || lower.includes('wireless')) return '/categories/iot';
    if (lower.includes('component') || lower.includes('motor') || lower.includes('stepper')) return '/categories/electronic-components';
      ...
      <main className="min-h-[70vh]">
        ...
        {activeTab === 'home' && (
          <>
            ...
            {/* 6. Deals & Offers */}
            <DealsAndOffersSection onShopDeals={() => handleTabChange('offers')} />

            {/* 7. Robotics Collection Showcase */}
            <RoboticsCollectionShowcase onExploreCollection={handleCategoryNavigation} />
        ...
        {activeTab === 'categories' && (
          <div className="py-10 animate-in fade-in duration-300">
            <ShopByCategory onSelectCategory={(catName) => handleTabChange('products', catName)} />
            <FeaturedCategories onSelectCategory={(catName) => handleTabChange('products', catName)} />
            <ProductGridSection
              onAddToCart={handleAddToCart}
              onToggleWishlist={handleToggleWishlist}
              onQuickView={(prod) => setSelectedQuickView(prod)}
              ...
              <div className="bg-[#00AEEF] text-white p-8 rounded-3xl shadow-xl flex flex-col md:flex-row justify-between items-center gap-6">
                <div>
                  ...
                  <h1 className="text-3xl font-extrabold mt-2">
                    {activeCategoryFilter ? `Filtered: ${activeCategoryFilter}` : 'Complete STEM Hardware Catalogue'}
                  </h1>
                  <p className="text-xs text-white/90 mt-1">100% Genuine Certified Microcontrollers, Drone Sensors, and DIY Kits.</p>
                </div>
          ...
          <div className="py-10 max-w-7xl mx-auto px-6 space-y-8 animate-in fade-in duration-300">
            <div className="bg-gradient-to-r from-[#0A1128] via-[#0F172A] to-[#1E56A0] text-white p-8 rounded-3xl border border-[#D4AF37]/30 shadow-2xl space-y-3">
              ...
              <p className="text-xs text-slate-300 max-w-2xl">
                Flight controllers, ESCs, BLDC Motors, MavLink GPS receivers, LiPo battery packs, and carbon propellers.
              </p>
            </div>
        ...
        {(activeTab === 'robotics-kits' || activeTab === 'arduino-iot' || activeTab === 'stem-kits') && (
          <div className="py-10 max-w-7xl mx-auto px-6 space-y-8 animate-in fade-in duration-300">
            <div className="bg-[#00AEEF] text-white p-8 rounded-3xl shadow-xl space-y-2">
              ...
              <p className="text-xs text-white/90">
                Verified hardware modules, official dev boards, and complete assembly manuals.
              </p>
            </div>
        ...
        {activeTab === 'offers' && (
          <div className="py-10 max-w-7xl mx-auto px-6 space-y-8 animate-in fade-in duration-300">
            <DealsAndOffersSection onShopDeals={() => handleTabChange('products')} />
            <ProductGridSection
              onAddToCart={handleAddToCart}
              onToggleWishlist={handleToggleWishlist}
              onQuickView={(prod) => setSelectedQuickView(prod)}
