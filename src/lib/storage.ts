...
export interface StorageObjectMetadata {
  ...
  url: string;
  isPrivate: boolean;
  context: 'PRODUCT_IMAGE' | 'SERVICE_IMAGE' | 'LEARNING_MEDIA' | 'OFFER_MEDIA' | 'SUPPORT_ATTACHMENT';
  createdAt: Date;
}
...
export class LocalStorageService implements StorageService {
  ...
  constructor() {
    this.publicPrefix = process.env.STORAGE_PUBLIC_URL_PREFIX || 'https://media.prayogindia.com';
  }
...
export class S3StorageService implements StorageService {
  ...
  constructor() {
    ...
    this.endpoint = process.env.STORAGE_ENDPOINT || `https://s3.${this.region}.amazonaws.com`;
  }
 ...
 */
export function generateStorageKey(context: string, userId: string, originalFilename: string): string {
  ...
  const randomUuid = Math.random().toString(36).substring(2, 10) + Date.now().toString(36);
