...
export async function getLearningResourceBySlug(
  ...
  return (
    LEARNING_RESOURCES.find((l) => l.slug === slug || l.id === slug) ||
...
export async function getCustomerOrders(
  page = 1,
  limit = 10,
): Promise<{ items: CustomerOrder[]; total: number; totalPages: number }> {
  try {
...
export async function getSupportTicketById(
  ...
  try {
    ...
    console.warn(
      "Falling back to local dataset for Support Ticket detail",
      err,
    );
  }

  return (
    MOCK_SUPPORT_TICKETS.find(
