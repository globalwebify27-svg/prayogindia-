...
const SENSITIVE_KEYS = ['password', 'passwordhash', 'token', 'secret', 'cookie', 'authorization'];
...
export const logger = {
  info: (message: string, meta?: Record<string, any>) => {
    console.log(JSON.stringify({ level: 'info', message, meta: redactSensitiveData(meta || {}), timestamp: new Date().toISOString() }));
  },
  warn: (message: string, meta?: Record<string, any>) => {
    console.warn(JSON.stringify({ level: 'warn', message, meta: redactSensitiveData(meta || {}), timestamp: new Date().toISOString() }));
  },
  error: (message: string, meta?: Record<string, any>) => {
    console.error(JSON.stringify({ level: 'error', message, meta: redactSensitiveData(meta || {}), timestamp: new Date().toISOString() }));
  },
};
