...
if (key1.includes("..") || key1.includes("malicious_script")) {
  throw new Error(
    "Path traversal vulnerability detected in storage key generation",
  );
}
...
if (validateMimeType("application/x-executable") !== false)
  throw new Error("Executable file type allowed erroneously");
if (validateMimeType("text/html") !== false)
  throw new Error("HTML file type allowed erroneously");
if (validateMimeType("image/png") !== true)
  throw new Error("Valid PNG image rejected");
if (validateMimeType("application/pdf") !== true)
  throw new Error("Valid PDF document rejected");
...
if (validateSize(15 * 1024 * 1024) !== false)
  throw new Error("15MB file allowed past 10MB limit");
if (validateSize(2 * 1024 * 1024) !== true)
  throw new Error("Valid 2MB file rejected");

// 4. Context Authorization Test
const validateCustomerContext = (context: string) =>
  context === "SUPPORT_ATTACHMENT";

if (validateCustomerContext("PRODUCT_IMAGE") !== false)
  throw new Error("Customer allowed to upload product images!");
if (validateCustomerContext("SERVICE_IMAGE") !== false)
  throw new Error("Customer allowed to upload service images!");
if (validateCustomerContext("SUPPORT_ATTACHMENT") !== true)
  throw new Error("Customer support attachment context rejected");
...
if (userBAccess.status !== 404) {
  throw new Error(
    "Customer isolation failed: User B was able to access User A private attachment",
  );
}

console.log(
  "✅ ALL B8 STORAGE, UPLOAD & ATTACHMENT VERIFICATION TESTS PASSED SUCCESSFULLY!",
);
