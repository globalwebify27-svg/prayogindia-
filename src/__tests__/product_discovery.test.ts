...
if (validatePriceRange("500", "200").valid !== false)
  throw new Error("Min price greater than max price allowed erroneously");
if (validatePriceRange("100", "1000").valid !== true)
  throw new Error("Valid price range rejected");
if (validatePriceRange("-50", "2000").min !== null)
  throw new Error("Negative price allowed");
...
if (sanitizeSort("DELETE FROM products;--") !== "newest")
  throw new Error("SQL Injection in sort param not sanitized!");
if (sanitizeSort("price-asc") !== "price-asc")
  throw new Error("Valid sort option rejected");
...
if (suggestions.length > 5)
  throw new Error("Suggestions exceeded request limit cap");
...
if (related.some((r) => r.id === targetProduct.id))
  throw new Error("Current product included inside related products");

console.log(
  "✅ ALL B10 SEARCH, FILTERING & PRODUCT DISCOVERY VERIFICATION TESTS PASSED SUCCESSFULLY!",
);
