...
import { generateStorageKey, ALLOWED_ATTACHMENT_MIME_TYPES } from '../lib/storage';
...
if (cleanEmail !== 'master.e2e@prayogindia.com') throw new Error('Auth: Email normalization failed');

const sanitizedUser = sanitizeUser({ ...masterTestUser, passwordHash: '$2b$12$secretHash...' } as any);
if ((sanitizedUser as any).passwordHash) throw new Error('Auth: Password hash leaked in user object');

// 2. Product Discovery & Multi-Filtering Test (B3, B10)
const matches = PRODUCTS.filter(p => p.name.toLowerCase().includes('drone') || p.category.toLowerCase().includes('drone'));
if (matches.length === 0) throw new Error('Products: Drone product search returned empty');

// 3. Storage & Upload Pre-signed Token Authorization Test (B8)
const safeStorageKey = generateStorageKey('SUPPORT_ATTACHMENT', masterTestUser.id, 'sensor_data.pdf');
if (!safeStorageKey.startsWith('support_attachment/usr-e2e-master/') || !safeStorageKey.endsWith('.pdf')) {
...
if (!renderedEmail.subject.includes('PRG-2026-8888') || !renderedEmail.html.includes('₹12,999')) {
...
const redactedLogs = redactSensitiveData({ password: 'secretPassword', token: 'jwtToken' });
if (redactedLogs.password !== '[REDACTED]' || redactedLogs.token !== '[REDACTED]') {
...
if (!rateLimitRes.allowed) throw new Error('Security: Initial rate limit bucket check failed');

console.log('✅ ALL MASTER E2E INTEGRATION TESTS PASSED SUCCESSFULLY (PHASES B1 - B12)!');
