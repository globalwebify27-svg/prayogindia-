...
if (redacted.password !== "[REDACTED]")
  throw new Error("Password was not redacted in logger!");
if (redacted.passwordHash !== "[REDACTED]")
  throw new Error("Password hash was not redacted in logger!");
if (redacted.token !== "[REDACTED]")
  throw new Error("Token was not redacted in logger!");
if (redacted.cookie !== "[REDACTED]")
  throw new Error("Cookie was not redacted in logger!");
if (redacted.safeField !== "Drone STEM Kit")
  throw new Error("Safe field was corrupted during log redaction!");
...
for (const res of resources) {
  ...
  if (verifyAccess(userB.id, res) !== 404) {
    throw new Error(
      `IDOR Vulnerability Detected! Bob granted access to Alice's ${res.type}`,
    );
  }
}

console.log(
  "✅ ALL B11 SECURITY, VALIDATION & IDOR PROTECTION VERIFICATION TESTS PASSED SUCCESSFULLY!",
);
