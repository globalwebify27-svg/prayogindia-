...
const sampleOffers = [
  {
    id: "off-1",
    slug: "monsoon-stem-deal",
    title: "Monsoon STEM Bundle",
    status: "Active",
    badge: "FEATURED",
  },
  {
    id: "off-2",
    slug: "uav-fest",
    title: "UAV Flight Controller Fest",
    status: "Upcoming",
    badge: "UPCOMING",
  },
];
...
if (getServiceBySlug("stem-lab-setup").status !== 200)
  throw new Error("Service slug resolution failed");
if (getServiceBySlug("invalid-slug-123").status !== 404)
  throw new Error("Service 404 handling failed");
...
const getActiveOffers = () =>
  sampleOffers.filter((o) => o.status === "Active" || o.status === "Upcoming");
if (getActiveOffers().length !== 2)
  throw new Error("Offer listing filtering failed");
...
console.log(
  "✅ ALL B6 SERVICES, LEARNING HUB & OFFERS VERIFICATION TESTS PASSED SUCCESSFULLY!",
);
