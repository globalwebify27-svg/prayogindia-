...
if (sanitized.role !== 'ADMIN') throw new Error('Admin role sanitization failed');
...
if (verifyAdminAccess(customerUser.role).status !== 403) {
  throw new Error('Security Breach! Customer user granted access to Admin Desk!');
}
...
if (validateProductInput('', 1200, 10) !== false) throw new Error('Empty product name allowed');
if (validateProductInput('Sensors Pack', -50, 10) !== false) throw new Error('Negative product price allowed');
if (validateProductInput('Sensors Pack', 1200, 10) !== true) throw new Error('Valid product creation input rejected');

// 4. Order Status Transition Validation Test
const ALLOWED_ORDER_STATUSES = ['PROCESSING', 'PACKED', 'SHIPPED', 'DELIVERED', 'CANCELLED'];
const isValidOrderStatus = (st: string) => ALLOWED_ORDER_STATUSES.includes(st);

if (isValidOrderStatus('SHIPPED') !== true) throw new Error('Valid status SHIPPED rejected');
if (isValidOrderStatus('PAID_FAKE') !== false) throw new Error('Fake status accepted');

console.log('✅ ALL ADMIN PANEL & DASHBOARD VERIFICATION TESTS PASSED SUCCESSFULLY!');
