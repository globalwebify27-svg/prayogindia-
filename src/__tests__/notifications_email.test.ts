...
if (!orderTemplate.subject.includes('PRG-2026-9999')) throw new Error('Order template subject missing order number');
if (!orderTemplate.html.includes('₹4,999')) throw new Error('Order template HTML missing grand total');
...
if (!supportTemplate.html.includes('I2C pullup resistors')) throw new Error('Support template snippet rendering failed');
...
const testResilientNotification = async () => {
  ...
  if (!result || !result.id) throw new Error('Notification creation failed when email dispatch errored');
  return true;
};
...
testResilientNotification().then(() => {
  ...
  const mockNotifications = [
    ...
    { id: 'notif-2', userId: userA.id, title: 'Support Reply', readAt: new Date() },
    { id: 'notif-3', userId: userB.id, title: 'Enquiry Received', readAt: null },
  ];

  const getUnreadForUser = (userId: string) =>
    mockNotifications.filter(n => n.userId === userId && n.readAt === null).length;

  if (getUnreadForUser(userA.id) !== 1) throw new Error('User A unread count calculation failed');
  if (getUnreadForUser(userB.id) !== 1) throw new Error('User B unread count calculation failed');
  ...
  if (accessNotification(userA.id, 'notif-1').status !== 200) throw new Error('Owner access failed');
  if (accessNotification(userA.id, 'notif-3').status !== 404) throw new Error('Customer isolation failed on notification feed');

  console.log('✅ ALL B9 NOTIFICATIONS & EMAIL SYSTEM VERIFICATION TESTS PASSED SUCCESSFULLY!');
});
