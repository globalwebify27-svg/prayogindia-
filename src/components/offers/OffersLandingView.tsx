...
export const OffersLandingView: React.FC = () => {
  const featuredOffer = OFFERS_DATA.find(o => o.badge === 'FEATURED') || OFFERS_DATA[0];
  ...
  const offerProductIds = Array.from(new Set(OFFERS_DATA.flatMap(o => o.productIds)));
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-10 animate-in fade-in duration-300">
      ...
      <div className="py-6 border-b border-slate-100 space-y-2">
        ...
        <p className="text-xs sm:text-sm text-slate-600 max-w-2xl leading-relaxed">
          Shop official promotional discounts, institutional bundle vouchers, and hardware deal pricing across microcontrollers and flight controllers.
        </p>
      </div>
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-extrabold text-slate-900 tracking-tight">Active Promotional Campaigns</h2>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {OFFERS_DATA.map((off) => (
            <div key={off.id} className="bg-white rounded-3xl border border-slate-200 p-6 space-y-3 shadow-2xs hover:border-[#00AEEF]/50 transition-colors">
              <div className="flex items-center justify-between">
                <span className={`text-[10px] font-black uppercase px-2.5 py-0.5 rounded-full ${
                  off.status === 'Active' ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-amber-50 text-amber-600 border border-amber-200'
                }`}>
                  {off.status} Campaign
                </span>
                <span className="text-[10px] text-slate-400 font-bold">Valid: {off.endDate}</span>
              </div>

              <h3 className="text-base font-extrabold text-slate-900">{off.title}</h3>
              <p className="text-xs text-slate-500 line-clamp-2">{off.shortDescription}</p>
              ...
              {off.couponCode && (
                <div className="text-xs font-bold text-slate-700 bg-slate-50 p-2 rounded-xl border border-slate-200 flex items-center justify-between">
                  <span>Voucher: <strong className="text-[#00AEEF] font-mono">{off.couponCode}</strong></span>
                  <span className="text-[10px] text-slate-400">At Checkout</span>
                </div>
              )}
            </div>
          ))}
      ...
      <div className="space-y-6 pt-4 border-t border-slate-100">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-extrabold text-slate-900 tracking-tight">Products Included in Active Deals</h2>
            <p className="text-xs text-slate-500">Shop hardware items with special catalogue offer pricing.</p>
          </div>
        ...
        {dealProducts.length === 0 ? (
          <div className="py-12 text-center bg-slate-50 rounded-3xl border border-slate-200 space-y-3 max-w-sm mx-auto">
            ...
            <h3 className="text-sm font-bold text-slate-900">No Offer Products Found</h3>
            <button
