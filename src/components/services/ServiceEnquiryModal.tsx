...
export const ServiceEnquiryModal: React.FC<ServiceEnquiryModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden flex items-center justify-center p-4">
      {/* Backdrop */}
      <div onClick={onClose} className="fixed inset-0 bg-slate-950/70 backdrop-blur-xs transition-opacity animate-in fade-in" />
      ...
      <div className="relative max-w-lg w-full bg-white rounded-3xl p-6 sm:p-8 shadow-2xl border border-slate-100 z-10 space-y-6 text-slate-900">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          ...
          <button onClick={onClose} className="p-1.5 rounded-full bg-slate-100 text-slate-400 hover:text-slate-900">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {!submitted ? (
          <form onSubmit={handleSubmit} className="space-y-4 text-xs">
            ...
            <div className="space-y-1">
              <label className="font-bold text-slate-700 block">Selected Service</label>
              <input
                type="text"
                disabled
                value={serviceName}
            ...
            <div className="space-y-1">
              <label className="font-bold text-slate-700 block">Institutional / Work Email</label>
              <input
                type="email"
                required
            ...
            <div className="space-y-1">
              <label className="font-bold text-slate-700 block">Requirement Details</label>
              <textarea
                required
                rows={3}
          ...
          <div className="text-center py-6 space-y-4">
            ...
            <h4 className="text-lg font-black text-slate-900">Enquiry Prepared Successfully</h4>
            <p className="text-xs text-slate-600 max-w-xs mx-auto leading-relaxed">
              Thank you {name}. Our mechatronics & lab setup engineering team will contact you at {phone}.
            </p>
