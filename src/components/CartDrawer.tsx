...
import { X, Trash2, Plus, Minus, ShoppingBag, ArrowRight, ShieldCheck, Tag, Heart, AlertTriangle, Check } from 'lucide-react';
        ...
        <div className="w-screen max-w-md bg-white shadow-2xl flex flex-col justify-between border-l border-slate-100 animate-in slide-in-from-right duration-300">
          ...
          <div className="p-4 sm:p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/80">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base sm:text-lg font-black text-slate-900">Your Cart</h2>
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-3 sm:space-y-4">
            ...
            {items.length === 0 ? (
              ...
              items.map((item) => {
                ...
                return (
                  <div key={`${item.product.id}-${item.variant?.id || 'std'}`} className="flex gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-200/80 items-center">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-slate-400 font-mono">
                        <span>{sku}</span>
                        {item.variant && <span className="text-[#00AEEF] font-bold">({item.variant.name})</span>}
                      </div>
                    ...
                    <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-lg p-0.5 shrink-0">
                      <button 
                        onClick={() => onUpdateQuantity(item.product.id, -1, item.variant?.id)}
            ...
            <div className="p-4 sm:p-6 border-t border-slate-100 bg-slate-50 space-y-3">
              ...
              {appliedCode && (
                <p className="text-[11px] font-bold text-emerald-600 flex items-center gap-1">
                  <Check className="w-3 h-3" /> Coupon {appliedCode} applied (-₹{appliedDiscount})
                </p>
              )}
              ...
              <div className="space-y-1.5 text-xs text-slate-500 pt-1">
                ...
                <div className="flex justify-between text-sm font-black text-slate-900 pt-2 border-t border-slate-200">
                  <span>Total</span>
                  <span className="text-[#00AEEF]">₹{total.toLocaleString('en-IN')}</span>
                </div>
              </div>
