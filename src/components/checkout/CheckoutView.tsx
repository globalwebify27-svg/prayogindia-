...
export const CheckoutView: React.FC = () => {
  ...
  const [customerMobile, setCustomerMobile] = useState(user?.phone?.replace(/\D/g, '').slice(-10) || '');
  ...
  React.useEffect(() => {
    if (user) {
      ...
      if (user.phone) setCustomerMobile(user.phone.replace(/\D/g, '').slice(-10));
  ...
  const handleMobileChange = (val: string) => {
    ...
    if (user && user.phone && user.phone.replace(/\D/g, '').slice(-10) === clean) {
      setIsPhoneVerified(true);
    } else {
  ...
  const [addrFormType, setAddrFormType] = useState<'Home' | 'Office' | 'Lab / College'>('Office');
  const [addrFormIsDefault, setAddrFormIsDefault] = useState(false);
  const [pincodeValidationMsg, setPincodeValidationMsg] = useState<string | null>(null);
  ...
  const [selectedDeliveryMethod, setSelectedDeliveryMethod] = useState<'standard' | 'ranchi_24h' | 'faster_express' | 'store_pickup'>('standard');
  ...
  const subtotal = cart.reduce((sum, item) => {
    ...
    const { unitPrice } = calculateCustomerPrice(rawPrice, activeCustomerType, item.quantity);
  ...
  const selectedAddr = addresses.find(a => a.id === selectedShippingAddressId);
  const deliveryCity = selectedAddr?.city || 'Bengaluru';
  const deliveryPincode = selectedAddr?.pincode || '560100';

  const totalWeightGrams = cart.reduce((sum, item) => sum + (item.quantity * 250), 0);
  ...
  const handleApplyCoupon = (code: string) => {
    ...
    const matchedCoupon = INITIAL_PROMO_COUPONS.find(item => item.code.toUpperCase() === c);
    if (matchedCoupon) {
      ...
      if (evaluation.valid) {
        ...
        setAppliedCoupon(`${matchedCoupon.code} (-₹${evaluation.discountAmount.toLocaleString()})`);
        setCouponError(null);
      } else {
        setCouponDiscount(0);
        setAppliedCoupon(null);
    ...
    if (validation.eligible) {
      const disc = Math.round((subtotal * (validation.discountPercent || 10)) / 100);
  ...
  const handleSaveAddress = (e: React.FormEvent) => {
    ...
    if (editingAddressId) {
      ...
      setAddresses(prev => {
        const next = addrFormIsDefault ? prev.map(a => ({ ...a, isDefault: false })) : [...prev];
        return [...next, newAddr];
      });
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!customerEmail.trim() || !customerEmail.includes('@')) {
      setValidationError('Please enter a valid Email Address for invoice delivery.');
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }
    ...
    if (!isPhoneVerified) {
      setValidationError('Please verify your mobile number via OTP before placing the order.');
    ...
    const selectedAddr = addresses.find(a => a.id === selectedShippingAddressId);
    if (!selectedAddr || !selectedAddr.street.trim() || !selectedAddr.city.trim() || selectedAddr.pincode.trim().length !== 6) {
      setValidationError('Please select or add a complete delivery address with a valid 6-digit PIN code.');
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }
    ...
    if (requireGSTInvoice) {
      if (!companyName.trim()) {
        setValidationError('Company / Institution Name is required for B2B GST Tax Invoice.');
        return;
      }
      if (!gstinNumber.trim() || gstinNumber.trim().length !== 15) {
        setValidationError('A valid 15-character GSTIN number is required for B2B Input Tax Credit.');
        return;
      }
    }
  ...
  if (isOrderPlacedDemo) {
    ...
    const { coinsEarned } = calculateEarnedRewards(subtotal, activeCustomerType);
      ...
      <div className="max-w-xl mx-auto py-16 px-4 text-center space-y-6 animate-in fade-in duration-300">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-slate-500">
            Thank you for ordering with Prayog India. Your hardware order has been registered for warehouse packaging and dispatch.
          </p>
        </div>
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 space-y-3 text-left">
          <div className="flex justify-between items-center border-b border-slate-200 pb-3 text-xs">
            ...
            <span className="font-mono font-black text-slate-900 text-sm">{placedOrderNumber}</span>
          </div>
          <div className="flex justify-between items-center border-b border-slate-200 pb-3 text-xs">
            <span className="text-slate-500 font-medium">Customer Type</span>
            <span className={`text-[10px] font-black uppercase px-2.5 py-0.5 rounded-full border ${typeRule.badgeBg} ${typeRule.badgeColor} ${typeRule.badgeBorder}`}>
              {activeCustomerType}
            </span>
          </div>
          <div className="flex justify-between items-center border-b border-slate-200 pb-3 text-xs">
            <span className="text-slate-500 font-medium">Tax Invoice Output</span>
            <span className="font-extrabold text-purple-700 text-xs">
              {requireGSTInvoice ? `GST B2B Tax Invoice (${gstinNumber})` : typeRule.invoice.label}
            </span>
          </div>
          <div className="flex justify-between items-center border-b border-slate-200 pb-3 text-xs">
          ...
          <div className="flex justify-between items-center border-b border-slate-200 pb-3 text-xs">
            ...
            <span className="font-bold text-slate-800 uppercase">{paymentMethod.toUpperCase()} (Confirmed)</span>
          </div>
          <div className="flex justify-between items-center text-xs">
            <span className="text-slate-500 font-medium">Dispatch Method</span>
            <span className="font-bold text-slate-800 uppercase">{effectiveDeliveryMethod.replace('_', ' ')}</span>
          </div>
        </div>
    ...
    return (
      <div className="max-w-md mx-auto py-20 text-center space-y-4">
        ...
        <p className="text-xs text-slate-500 max-w-xs mx-auto">
          Please add microcontrollers, sensors, or robotics kits before opening checkout.
        </p>
        <Link
          href="/products"
  ...
  if (!isLoggedIn) {
    return (
      <div className="max-w-xl mx-auto py-12 px-4 sm:px-6 space-y-6 animate-in fade-in duration-300">
        <CategoryBreadcrumb items={[{ label: 'Cart', href: '/cart' }, { label: 'Verification Required' }]} />
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-xl text-center space-y-6">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-slate-500 max-w-md mx-auto leading-relaxed">
              To prevent fraudulent orders, guarantee GST tax invoicing, and track courier dispatch, please sign in to your account or verify your 10-digit mobile number with an OTP.
            </p>
          </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8 animate-in fade-in duration-300">
      {/* Breadcrumb */}
      <CategoryBreadcrumb items={[{ label: 'Cart', href: '/cart' }, { label: 'One-Page Checkout' }]} />
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-[#00AEEF] bg-[#E0F7FC] px-3 py-1 rounded-full border border-[#00AEEF]/20">
              Section 17 · Optimized One-Page Checkout
            </span>
          </div>
        ...
        <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 p-1.5 rounded-2xl">
          <span className="text-[11px] font-bold text-slate-500 pl-2">Segment:</span>
          <select
          ...
          >
            ...
            <option value="Registered Customer">Registered Customer (Loyalty 5%)</option>
            <option value="Walk-in Customer">Walk-in Customer (Store POS)</option>
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          {validationError && (
            <div className="bg-red-50 border-2 border-red-300 rounded-2xl p-4 flex items-start gap-3 text-red-900 text-xs shadow-md animate-in fade-in">
              ...
              <div>
                <h4 className="font-black text-red-950">Validation Error — Action Required</h4>
                <p className="text-red-800 font-semibold mt-0.5 leading-relaxed">{validationError}</p>
              </div>
            </div>
          )}
            ...
            <div className="bg-gradient-to-r from-[#0F172A] to-slate-900 text-white rounded-3xl p-5 border border-slate-700 shadow-md flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="text-xs font-black text-white">Have a Prayog Account?</h4>
                </div>
                <p className="text-[11px] text-slate-300 leading-relaxed">
                  Sign in or register to auto-fill verified addresses, access wholesale/student discounts, and earn Prayog Coins.
                </p>
              </div>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 space-y-4 shadow-2xs">
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider flex items-center gap-2 border-b border-slate-100 pb-3">
              <UserCheck className="w-4 h-4 text-[#00AEEF]" /> 1. Customer Contact Details
            </h3>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  {isPhoneVerified ? (
                    ...
                    >
                      <span>{otpSending ? 'Sending OTP...' : 'Verify with OTP →'}</span>
                    </button>
                  )}
                </div>
                ...
                <div className="bg-white rounded-3xl p-6 max-w-sm w-full border border-slate-200 shadow-2xl space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h4 className="text-sm font-black text-slate-900">Verify Mobile Phone</h4>
                        <p className="text-[11px] text-slate-500 font-semibold">+91 {customerMobile}</p>
                      </div>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="space-y-1.5 text-xs">
                    <label className="font-extrabold text-slate-700 block">Enter 6-Digit Code</label>
                    <input
                      type="text"
                      maxLength={6}
                      value={checkoutOtp}
            ...
            <div className="pt-2 border-t border-slate-100 space-y-3">
              ...
              {requireGSTInvoice && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-purple-50/60 p-4 rounded-2xl border border-purple-200 animate-in fade-in">
                  <div>
                    <label className="block text-[11px] font-bold text-purple-900 mb-1">Company / Institution Name *</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-purple-900 mb-1">GSTIN Number * (15 Digits)</label>
                    <input
                      type="text"
                      maxLength={15}
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 space-y-4 shadow-2xs">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider flex items-center gap-2">
                <MapPin className="w-4 h-4 text-[#00AEEF]" /> 2. Delivery Address
              </h3>
              <button
                ...
                return (
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <h4 className="text-xs font-extrabold text-slate-900 mt-1">{addr.name}</h4>
                      <p className="text-[11px] text-slate-600 leading-relaxed line-clamp-2">{addr.street}</p>
                      <p className="text-[11px] font-bold text-slate-700">
                        {addr.city}, {addr.state} - <span className="font-mono text-[#00AEEF]">{addr.pincode}</span>
                      </p>
                      <p className="text-[10px] text-slate-400 font-mono">{addr.phone}</p>
                    </div>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 space-y-4 shadow-2xs">
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider flex items-center gap-2 border-b border-slate-100 pb-3">
              <Truck className="w-4 h-4 text-[#00AEEF]" /> 3. Dispatch &amp; Courier Mode
            </h3>
            ...
            {hasRestrictedBatteryItem && (
              <div className="bg-amber-50 border-2 border-amber-300 rounded-2xl p-4 text-xs text-amber-950 space-y-1.5 shadow-xs">
                ...
                <p className="text-amber-900 font-medium">
                  <strong>Battery Item DGCA Rule:</strong> Air Express courier is restricted for safety. All items in this order will be dispatched together via Ground Surface Cargo.
                </p>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-extrabold text-slate-900 flex items-center gap-1">
                    <Truck className="w-3.5 h-3.5 text-amber-600" /> Standard Delivery
                  </span>
                  <span className={`text-[10px] font-black ${isFreeStandardShipping ? 'text-emerald-600' : 'text-slate-900'}`}>
                    {isFreeStandardShipping ? 'FREE' : '₹99'}
                  </span>
                </div>
                <p className="text-[11px] font-bold text-slate-700">Timeline: 7–10 Days</p>
                <p className="text-[10px] text-slate-500 mt-0.5">
                  {isFreeStandardShipping ? 'FREE Delivery applied (Order > ₹2,000)' : 'Free for orders above ₹2,000'}
                </p>
              </div>
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-extrabold text-slate-900 flex items-center gap-1">
                    <Zap className="w-3.5 h-3.5 text-emerald-600" /> Ranchi City Fast Track
                  </span>
                  <span className="text-[10px] font-black text-emerald-600">
                    {isFreeStandardShipping ? 'FREE' : '₹49'}
                  </span>
                </div>
                <p className="text-[11px] font-bold text-emerald-700">Within 24 Hours</p>
                <p className="text-[10px] text-slate-500 mt-0.5">
                  {isRanchiEligible ? 'Eligible for Ranchi address (PIN 834xxx)' : 'Only for Ranchi City address'}
                </p>
              </div>
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-extrabold text-slate-900 flex items-center gap-1">
                    <Plane className="w-3.5 h-3.5 text-blue-600" /> Faster Delivery
                  </span>
                ...
                <p className="text-[11px] font-bold text-blue-700">
                  {hasRestrictedBatteryItem ? '2–4 Days (Surface)' : '24–48 Hours (Air)'}
                </p>
                <p className="text-[10px] text-slate-500 mt-0.5">Priority express courier by weight &amp; zone</p>
              </div>
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-extrabold text-slate-900 flex items-center gap-1">
                    <MapPin className="w-3.5 h-3.5 text-purple-600" /> Store Pickup
                  </span>
                  <span className="text-[10px] font-black text-emerald-600">FREE</span>
                </div>
                <p className="text-[11px] font-bold text-slate-700">Same-Day Pickup</p>
                <p className="text-[10px] text-slate-500 mt-0.5">Collect from local Prayog desk</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 space-y-4 shadow-2xs">
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider flex items-center gap-2 border-b border-slate-100 pb-3">
              <CreditCard className="w-4 h-4 text-[#00AEEF]" /> 4. Payment Gateway
            </h3>

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { id: 'upi', label: 'UPI / QR Code', icon: <QrCode className="w-4 h-4 text-emerald-600" />, desc: 'GPay, PhonePe, Paytm' },
                { id: 'card', label: 'Debit / Credit Card', icon: <CreditCard className="w-4 h-4 text-blue-600" />, desc: 'Visa, Master, RuPay' },
                { id: 'netbanking', label: 'Net Banking', icon: <Landmark className="w-4 h-4 text-purple-600" />, desc: '50+ Indian Banks' },
                { id: 'cod', label: 'Cash on Delivery', icon: <Banknote className="w-4 h-4 text-amber-600" />, desc: 'Pay at Doorstep' },
              ].map(opt => (
                <button
                  key={opt.id}
                  type="button"
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    {opt.icon}
                    {paymentMethod === opt.id && <Check className="w-3.5 h-3.5 text-[#00AEEF]" />}
                  </div>
                  <div>
        ...
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto animate-in fade-in">
          <div className="bg-white rounded-3xl border border-slate-200 w-full max-w-lg shadow-2xl overflow-hidden my-8">
            ...
            <form onSubmit={handleSaveAddress} className="p-6 space-y-4 text-xs">
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">Street Address, Lab / Room # *</label>
                <textarea
                  required
                  rows={2}
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block font-bold text-slate-700 mb-1">PIN Code * (6 digits)</label>
                  <input
                    type="text"
                    required
                    maxLength={6}
              ...
              {pincodeValidationMsg && (
                <p className="text-red-500 text-[11px] font-bold">{pincodeValidationMsg}</p>
              )}
