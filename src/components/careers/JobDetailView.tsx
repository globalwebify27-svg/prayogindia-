...
export const JobDetailView: React.FC<JobDetailProps> = ({ slug }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-10 animate-in fade-in duration-300">
      {/* 1. Breadcrumb */}
      <CategoryBreadcrumb
        items={[{ label: "Careers", href: "/careers" }, { label: job.title }]}
      />
          ...
          <section className="space-y-3">
            ...
            <ul className="space-y-2">
              {job.responsibilities.map((resp, idx) => (
                <li
                  key={idx}
                  className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 text-xs font-semibold text-slate-700 flex items-start gap-2.5"
                >
        ...
        <div className="lg:col-span-5 bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-2xs space-y-6">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Submit your engineering application profile.
            </p>
          </div>
          ...
          {!submitted ? (
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              ...
              <div className="space-y-1">
                <label className="font-bold text-slate-700 block">
                  Cover Note / GitHub / Portfolio Link
                </label>
                <textarea
                  required
                  rows={3}
            ...
            <div className="text-center py-8 space-y-3">
              ...
              <p className="text-xs text-slate-600 max-w-xs mx-auto">
                Thank you {name}. You can also email your CV directly to{" "}
                <strong className="text-slate-900">{COMPANY_INFO.email}</strong>{" "}
                referencing role: {job.title}.
              </p>
            </div>
          )}
        </div>
