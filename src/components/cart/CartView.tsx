...
export const CartView: React.FC = () => {
  ...
  const [deliveryMethod, setDeliveryMethod] = useState<DeliveryMethod>('surface');
  const [showRestrictionModal, setShowRestrictionModal] = useState(false);
  const [restrictionModalShown, setRestrictionModalShown] = useState(false);

  // ── Stock Validation Check
  const outOfStockItems = cart.filter(item => {
    const isAvailable = item.variant ? item.variant.inStock : item.product.inStock;
    return !isAvailable;
  });
  ...
  const hasBatteryOrHazardous = cart.some(item => {
    ...
    return tag === 'Battery Item' || tag === 'Hazardous' || name.includes('battery') || name.includes('lipo');
  });
  ...
  const effectiveDeliveryMethod = hasBatteryOrHazardous && deliveryMethod === 'express'
    ? 'surface'
    : deliveryMethod;

  // ── Customer Type & Promotional Pricing Matrix
  const currentCustomerType: CustomerType = user?.customerType || 'Guest Customer';
  const customerRule = Object.values(CUSTOMER_TYPE_RULES).find(r => r.name === currentCustomerType);
  ...
  const segmentDiscount = customerDiscountPercent > 0
  ...
  // Delivery fee
  const deliveryFee = effectiveDeliveryMethod === 'store_pickup'
    ? 0
  ...
  const handleRedeemPoints = () => {
    ...
    if (!user || user.rewardPoints < pts) {
      setPointsError(`Insufficient coins. You have ${user?.rewardPoints || 0} coins.`);
      return;
    }
  ...
  const handleCheckPincode = () => {
    if (pincode.trim().length === 6 && /^\d+$/.test(pincode)) {
      if (hasBatteryOrHazardous) {
        setPincodeStatus(`PIN ${pincode} verified: Surface Cargo Ground Delivery (3-5 business days).`);
      } else {
        setPincodeStatus(`PIN ${pincode} verified: Priority Air Express Dispatch (24-48 hours).`);
      }
    } else {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        ...
        <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 p-1.5 rounded-2xl">
          <span className="text-[11px] font-bold text-slate-500 pl-2">Pricing Tier:</span>
          <select
          ...
          >
            ...
            <option value="Registered Customer">Registered Customer (5% Loyalty)</option>
        ...
        <div className="py-24 text-center bg-gradient-to-br from-slate-50 to-white rounded-3xl border border-slate-200 space-y-4 max-w-lg mx-auto my-8">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500 max-w-xs mx-auto leading-relaxed">
              Explore our catalogue of Arduino boards, drone kits, LiDAR sensors, and STEM robotics hardware.
            </p>
          </div>
          <Link
            href="/products"
          ...
          <div className="lg:col-span-8 space-y-6">
              ...
              <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-start gap-3 text-red-900 text-xs animate-in fade-in">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-red-700">
                    {outOfStockItems.length} item(s) in your cart are currently out of stock. Please remove or move them to wishlist before checkout.
                  </p>
                </div>
              </div>
            )}
            ...
            {hasBatteryOrHazardous && (
              <div className="bg-amber-50 border-2 border-amber-300 rounded-3xl p-5 space-y-2.5 text-amber-950 text-xs shadow-xs animate-in fade-in">
                ...
                <p className="text-amber-900 font-medium leading-relaxed">
                  Your cart contains mixed hardware (e.g. Flight Controllers, Microcontrollers, and Lithium/LiPo Batteries). Because aviation safety regulations prohibit flying lithium cells, the <strong>entire consignment will be safely transported via Surface Ground Logistics</strong>.
                </p>

                <div className="bg-white/80 border border-amber-200 rounded-xl p-2.5 flex items-center justify-between text-[11px] font-bold">
                  <span className="text-slate-600">Restricted Item Detected:</span>
                  <span className="text-amber-900 font-extrabold font-mono">
                    {cart.find(i => i.product.shippingTag === 'Battery Item' || i.product.name.toLowerCase().includes('battery') || i.product.name.toLowerCase().includes('lipo'))?.product.name || 'LiPo Battery Pack'}
                  </span>
                </div>
              </div>
            )}
              ...
              <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-3.5 flex items-center justify-between gap-3 text-xs text-emerald-900">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-black">
                    {currentCustomerType} Privilege: {customerDiscountPercent}% instant discount applied!
                  </span>
                </div>
                ...
                return (
                  <div
                    ...
                    className={`bg-white rounded-3xl border p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all shadow-2xs ${
                      !isInStock ? 'border-red-200 bg-red-50/20' : 'border-slate-200 hover:border-[#00AEEF]/40'
                    }`}
                  >
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-3 w-full sm:w-auto border-t sm:border-t-0 border-slate-100 pt-3 sm:pt-0">
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => moveToWishlist(item.product, item.variant?.id)}
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-6 space-y-4 shadow-2xs">
              <h4 className="text-xs font-black uppercase text-slate-900 flex items-center gap-1.5">
                <Truck className="w-4 h-4 text-[#00AEEF]" /> Select Dispatch &amp; Delivery Mode
              </h4>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-extrabold text-xs text-slate-900 flex items-center gap-1">
                      <Plane className="w-3.5 h-3.5 text-blue-600" /> Air Freight
                    </span>
                    <span className={`text-[10px] font-black uppercase ${hasBatteryOrHazardous ? 'text-red-600' : 'text-emerald-600'}`}>
                      {hasBatteryOrHazardous ? '❌ Unavailable' : 'FREE >₹999'}
                    </span>
                  </div>
                  {hasBatteryOrHazardous ? (
                    <div className="space-y-0.5 mt-1">
                      <p className="text-[10px] text-red-700 font-extrabold">❌ Air Freight – Unavailable</p>
                      <span className="text-[9px] text-slate-500 font-bold block">Reason: Contains Battery Products</span>
                    </div>
                  ) : (
                    <p className="text-[10px] text-slate-500">24-48 Hours pan-India flight cargo</p>
                  )}
                </button>
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-extrabold text-xs text-slate-900 flex items-center gap-1">
                      <Truck className="w-3.5 h-3.5 text-emerald-700" /> Surface Freight
                    </span>
                    <span className="text-[10px] font-black text-emerald-700 uppercase">
                      ✔ Surface Freight
                    </span>
                  </div>
                  <p className="text-[10px] text-slate-500">3-5 days ground freight (DGCA Battery Certified)</p>
                </button>
                ...
                >
                  ...
                  <p className="text-[10px] text-slate-500">Collect from nearest tech desk</p>
                </button>
              </div>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-3xl p-5 space-y-3">
              <h4 className="text-xs font-black uppercase text-slate-900 flex items-center gap-1.5">
                <MapPin className="w-4 h-4 text-[#00AEEF]" /> Check Delivery Timelines by PIN Code
              </h4>
              ...
              {pincodeStatus && (
                <p className="text-xs font-bold text-[#00AEEF] flex items-center gap-1">
                  <CheckCircle2 className="w-4 h-4 text-emerald-500" /> {pincodeStatus}
                </p>
              )}
            </div>
          </div>
            ...
            <div className="bg-[#0F172A] text-white rounded-3xl p-6 sm:p-7 border border-slate-800 shadow-2xl space-y-6">
                ...
                <div className="space-y-2 pt-2 border-t border-slate-800">
                  <div className="flex items-center justify-between text-xs">
                    <label className="font-bold text-slate-300 flex items-center gap-1">
                      <Coins className="w-3.5 h-3.5 text-amber-400" /> Prayog Coins
                    </label>
                    <span className="text-[11px] text-amber-300 font-bold">
                      {user.rewardPoints} Available (₹{(user.rewardPoints * 0.5).toFixed(0)})
                    </span>
                  </div>
                  <div className="flex gap-2">
                    <input
                  ...
                  {pointsError && (
                    <p className="text-xs font-bold text-red-400">{pointsError}</p>
                  )}
                </div>
              )}
              ...
              <div className="space-y-2.5 text-xs text-slate-300 pt-3 border-t border-slate-800">
                ...
                <div className="flex justify-between text-slate-400">
                  ...
                  <span className="text-emerald-400 font-bold">₹{gstAmount.toLocaleString('en-IN')} (Incl.)</span>
                </div>

                <div className="flex justify-between text-slate-400">
                  <span>Delivery ({effectiveDeliveryMethod.replace('_', ' ')})</span>
                  <span className={deliveryFee === 0 ? 'text-emerald-400 font-extrabold' : 'text-white font-bold'}>
              ...
              <div className="space-y-2">
                ...
                <div className="text-[10px] text-slate-400 text-center flex items-center justify-center gap-1 pt-1">
                  ...
                  <span>256-Bit Encrypted Payment Gateway &amp; Tax Invoice</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {showRestrictionModal && (
        <div className="fixed inset-0 z-50 overflow-hidden flex items-center justify-center p-4">
          <div onClick={() => setShowRestrictionModal(false)} className="fixed inset-0 bg-slate-950/75 backdrop-blur-xs animate-in fade-in" />
          <div className="relative max-w-md w-full bg-white rounded-3xl p-6 sm:p-7 shadow-2xl z-10 space-y-4 text-xs animate-in zoom-in-95">
            ...
            <div className="bg-amber-50/80 border border-amber-200 rounded-2xl p-4 space-y-2 text-slate-800 font-medium leading-relaxed">
              ...
              <p className="text-slate-600 text-xs">
                Your cart contains lithium battery cells or flight battery hardware. Please select <strong>Surface Mode</strong> for delivery.
              </p>
            </div>

            {/* Visual Restriction Switch */}
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3.5 space-y-2">
              <span className="text-[10px] font-black uppercase text-slate-500 block">Automatic Restriction Applied:</span>
              ...
              <div className="flex items-center justify-between text-xs py-1">
                <span className="font-bold text-slate-900 flex items-center gap-1.5">
                  <Truck className="w-4 h-4 text-emerald-600" /> Surface Freight:
                </span>
