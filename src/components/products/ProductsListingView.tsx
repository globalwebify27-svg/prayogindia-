...
import { ProductToolbar, SortOption } from '@/components/products/ProductToolbar';
import { ProductFilters, TechnicalSpecFilters } from '@/components/products/ProductFilters';
...
const POPULAR_SEARCHES = [
  'Arduino UNO R4', 'ESP32-S3', 'Raspberry Pi 5', 'Pixhawk 6C',
  'LiPo 4S', 'LiDAR RPLiDAR', 'MPU6050', 'Mecanum Wheel', 'BLDC Motor 1000KV',
  'NRF24L01', 'HC-SR04', 'MG996R Servo', 'SpeedyBee F405',
];
...
function ProductsContent() {
  ...
  const { cart, wishlist, addToCart: storeAddToCart, toggleWishlist: storeToggleWishlist } = useStore();
  ...
  const handleTechSpecChange = (key: keyof TechnicalSpecFilters, value: string | number | null) => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedBrand) {
      result = result.filter(
        p => (p.brand ?? p.name).toLowerCase().includes(selectedBrand.toLowerCase())
      );
    }
    ...
    if (techSpecs.rpmKv) {
      ...
      result = result.filter(p =>
        p.name.toLowerCase().includes(r) || p.description.toLowerCase().includes(r)
      );
    }
    ...
    if (techSpecs.material) {
      ...
      result = result.filter(p =>
        p.name.toLowerCase().includes(m) || p.description.toLowerCase().includes(m)
      );
    }
    if (techSpecs.minRating) {
      result = result.filter(p => p.rating >= (techSpecs.minRating as number));
    }
    if (techSpecs.minDiscount) {
      result = result.filter(p => {
        const pct = p.mrp > p.price
    ...
    switch (sortBy) {
      ...
      case 'popularity':
        ...
        break;
      case 'best-selling':
        result = [...result].sort((a, b) => (b.reviews * b.rating) - (a.reviews * a.rating));
        break;
      case 'discount':
    ...
    return result;
  }, [searchQuery, selectedCategory, selectedBrand, inStockOnly, priceRange, techSpecs, sortBy]);
    ...
    <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2.5 sm:py-4 space-y-3 sm:space-y-4 animate-in fade-in duration-300">
            ...
            <div className="space-y-3">
              {filteredProducts.slice(0, visibleCount).map(product => {
                const discountPct = product.mrp > product.price
                  ...
                  : null;
                const specPills = Object.entries(product.specs ?? {}).slice(0, 4);
                    ...
                    <div className="flex-1 min-w-0 space-y-1.5">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <h3 className="text-xs font-bold text-slate-900 line-clamp-2 leading-snug">{product.name}</h3>
                        </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {specPills.map(([key, val]) => (
                          <span key={key} className="bg-slate-100 text-slate-600 text-[9px] font-bold px-1.5 py-0.5 rounded truncate">
                            {key}: {val}
                          </span>
                        ))}
                      </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className={`text-[10px] font-bold cursor-pointer ${
                              wishlistIds.has(product.id) ? 'text-red-500' : 'text-slate-500 hover:text-red-500'
                            }`}
                          >
      ...
      {mobileFilterOpen && (
        <div className="fixed inset-0 z-50 overflow-hidden flex justify-end">
          <div onClick={() => setMobileFilterOpen(false)} className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs" />
          <div className="relative w-full max-w-xs bg-white h-full p-6 overflow-y-auto shadow-2xl space-y-4 z-10">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200">
              <h3 className="text-sm font-black text-slate-900 uppercase">Filter Products</h3>
            ...
            <ProductFilters
              ...
              techSpecs={techSpecs}
              onTechSpecChange={handleTechSpecChange}
              onClearAll={() => { clearAllFilters(); setMobileFilterOpen(false); }}
            />
          </div>
        </div>
      )}
