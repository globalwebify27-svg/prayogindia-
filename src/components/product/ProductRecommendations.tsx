...
import { Star, ShoppingCart, Zap, Heart, ChevronRight, Package } from 'lucide-react';
...
const RecoCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  return (
    <div className="group bg-white border border-slate-200 rounded-2xl overflow-hidden hover:border-[#00AEEF]/50 hover:shadow-lg transition-all duration-300 flex flex-col">
      ...
      {/* Image */}
      <Link href={`/products/${product.slug || product.id}`} className="block relative h-36 bg-slate-50 overflow-hidden">
        <Image
          src={product.image}
          alt={product.name}
          fill
      ...
      <div className="p-3 flex-1 flex flex-col gap-2">
        <span className="text-[8px] font-mono font-bold text-slate-400 uppercase">{product.sku}</span>
...
const FrequentlyBoughtTogetherSection: React.FC<{ products: Product[] }> = ({ products }) => {
  ...
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 space-y-5">
      <div>
        <h3 className="text-sm font-black text-slate-900 uppercase tracking-tight">Frequently Bought Together</h3>
        <p className="text-xs text-slate-500 mt-0.5">Customers who bought this item also purchased:</p>
      </div>
          ...
          <React.Fragment key={prod.id}>
            <div className="flex flex-col items-center gap-1.5 shrink-0">
              <div className="relative w-20 h-20 bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-2xs">
                <Image src={prod.image} alt={prod.name} fill className="object-contain p-1.5" />
              </div>
              <span className="text-[9px] font-bold text-slate-700 text-center max-w-[80px] line-clamp-2 leading-tight">
                {prod.name.split('(')[0].trim()}
              </span>
              <span className="text-[10px] font-extrabold text-[#00AEEF]">₹{prod.price.toLocaleString('en-IN')}</span>
            </div>
            {idx < products.length - 1 && (
              <span className="text-xl font-black text-slate-300 shrink-0">+</span>
            )}
          </React.Fragment>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between gap-4 bg-white border border-slate-200 rounded-2xl p-4">
        <div>
          <div className="text-[10px] text-slate-400 font-semibold">Bundle Total ({products.length} items)</div>
          <div className="flex items-baseline gap-2">
            <span className="text-xl font-black text-slate-900">₹{bundleTotal.toLocaleString('en-IN')}</span>
            <span className="text-sm text-slate-400 line-through">₹{bundleMrp.toLocaleString('en-IN')}</span>
...
export const ProductRecommendations: React.FC<ProductRecommendationsProps> = ({ groups }) => {
  const [activeTab, setActiveTab] = useState(groups[0]?.id ?? '');

  const validGroups = groups.filter(g => g.products.length > 0);
  if (validGroups.length === 0) return null;

  const activeGroup = validGroups.find(g => g.id === activeTab) ?? validGroups[0];
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex flex-wrap gap-2">
          {validGroups.map(group => (
            ...
            >
              {group.label}
              <span className={`ml-1.5 text-[9px] font-black ${activeTab === group.id ? 'text-[#00AEEF]' : 'text-slate-400'}`}>
