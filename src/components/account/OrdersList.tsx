...
import { ShoppingBag, Eye, Truck, CheckCircle2, Clock, FileText, ArrowRight } from 'lucide-react';
...
export const OrdersList: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-2xs space-y-6 text-slate-900">
      ...
      <div>
        <h2 className="text-xl font-extrabold text-slate-900 tracking-tight">My Order History</h2>
        <p className="text-xs text-slate-500">Track current hardware shipments, view GST tax invoices, and reorder STEM supplies.</p>
      </div>

      {orders.length === 0 ? (
        <div className="text-center py-12 space-y-3">
          <ShoppingBag className="w-12 h-12 text-slate-300 mx-auto" />
          <p className="text-xs text-slate-500 font-bold">You have not placed any orders yet.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {orders.map((ord) => (
            <div key={ord.id} className="bg-slate-50 border border-slate-200/90 rounded-2xl p-5 space-y-4 shadow-2xs">
                  ...
                  <div key={idx} className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <span className="text-[10px] text-slate-400 font-mono">SKU: {item.sku} | Qty: {item.quantity}</span>
                      </div>
                    </div>
                    <span className="text-xs font-bold text-slate-900 shrink-0">₹{item.price.toLocaleString()}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-200/80">
                ...
                <button
                  onClick={() => alert(`Downloading Official GST Tax Invoice for Order ${ord.orderNumber}...`)}
