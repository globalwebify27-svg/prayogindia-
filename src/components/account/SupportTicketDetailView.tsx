...
import { Headphones, Plus, Send, MessageSquare, CheckCircle2, ArrowLeft, Clock } from 'lucide-react';

interface SupportTicketDetailProps {
  ticketId: string;
}

export const SupportTicketDetailView: React.FC<SupportTicketDetailProps> = ({ ticketId }) => {
  const [tickets, setTickets] = useState<SupportTicket[]>(MOCK_SUPPORT_TICKETS);
  const ticket = tickets.find(t => t.id === ticketId || t.ticketNumber === ticketId) || tickets[0];
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-2xs space-y-6 text-slate-900 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
        <div>
          <Link href="/account/support" className="text-xs font-bold text-[#00AEEF] hover:underline flex items-center gap-1 mb-1">
            <ArrowLeft className="w-3.5 h-3.5" /> Back to Support Tickets
          </Link>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-black text-slate-900">{ticket.ticketNumber}</h1>
            <span className={`text-xs font-black px-3 py-0.5 rounded-full border ${getStatusBadge(ticket.status)}`}>
              {ticket.status}
            </span>
          </div>
          <p className="text-xs text-slate-500 font-semibold mt-0.5">Category: {ticket.category} • Created {ticket.createdDate}</p>
        </div>
      </div>

      {/* Ticket Subject */}
      <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl">
        <span className="text-[10px] font-black uppercase text-slate-400 block">Subject Title</span>
        <h3 className="text-base font-extrabold text-slate-900">{ticket.subject}</h3>
      </div>

      {/* Messages Conversation Thread */}
      <div className="space-y-4">
        <h3 className="text-xs font-black uppercase text-slate-900 tracking-wider">Conversation Log</h3>
      ...
      <form onSubmit={handleSendReply} className="space-y-3 pt-2">
        <label className="font-bold text-xs text-slate-700 block">Reply to Support Desk</label>
