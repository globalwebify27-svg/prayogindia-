export interface InterStoreTransfer {
  ...
  dispatchedAt?: string;
  receivedAt?: string;
  status: 'Draft / Requested' | 'Approved' | 'Dispatched & In Transit' | 'Received & Stock Updated' | 'Cancelled';
...
export interface StockAdjustmentEntry {
  ...
  newStock: number;
  reason: 'Damaged goods' | 'Lost goods' | 'Physical count correction' | 'Returns' | 'Internal consumption' | 'Manual correction';
  adjustedBy: string;
  timestamp: string;
  notes: string;
}
