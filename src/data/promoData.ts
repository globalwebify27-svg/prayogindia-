...
export const INITIAL_PROMO_COUPONS: PromoCoupon[] = [
  ...
  },
  {
    id: 'coup-6',
    code: 'ROBOTICS25',
    description: '25% Category Clearance on Robotics & Drone Flight Controllers',
    discountType: 'percentage',
    discountValue: 25,
    minOrderValue: 4000,
...
export function evaluatePromoCoupon(
  ...
  if (coupon.expiryDate < today) {
    return { valid: false, discountAmount: 0, message: `Promo code expired on ${coupon.expiryDate}.` };
  }

  // Check Global Usage Limit
  if (coupon.usageCount >= coupon.usageLimitGlobal) {
    return { valid: false, discountAmount: 0, message: 'Promo code global usage limit has been reached.' };
  }
  ...
  if (coupon.customerTypeScope === 'B2B-only' && customerType !== 'B2B Customer') {
    return { valid: false, discountAmount: 0, message: 'This coupon is exclusive to verified B2B & Institutional accounts.' };
  }

  if (coupon.customerTypeScope === 'B2C-only' && customerType !== 'B2C Customer') {
    return { valid: false, discountAmount: 0, message: 'This coupon is exclusive to Retail B2C customers.' };
  }

  if (coupon.customerTypeScope === 'Walk-in-only' && !isWalkInPos) {
    return { valid: false, discountAmount: 0, message: 'This coupon is only valid for in-store Walk-in POS checkout.' };
  }

  if (coupon.customerTypeScope === 'Registered-customer' && !userEmail) {
    return { valid: false, discountAmount: 0, message: 'Please sign in to your registered account to use this coupon.' };
  }

  if (coupon.customerTypeScope === 'New-customer' && hasPreviousOrders) {
    return { valid: false, discountAmount: 0, message: 'This welcome coupon is only valid on your first order.' };
  }

  // Check Whitelist
  if (coupon.restrictedUserEmails && coupon.restrictedUserEmails.length > 0) {
    if (!userEmail || !coupon.restrictedUserEmails.map(e => e.toLowerCase()).includes(userEmail.toLowerCase())) {
      return { valid: false, discountAmount: 0, message: 'Your account is not eligible for this exclusive coupon code.' };
    }
  }
