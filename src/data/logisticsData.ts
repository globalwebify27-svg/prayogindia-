...
export interface ShipmentManifest {
  ...
  status: 'Manifest Created' | 'Pickup Scheduled' | 'In Transit' | 'Out for Delivery' | 'Delivered' | 'Cancelled' | 'RTO';
 ...
 */
export function checkPincodeServiceability(pincode: string, weightKg: number = 0.5): PinServiceabilityResult {
  ...
  let state = 'India';
  if (pin.startsWith('834')) { city = 'Ranchi'; state = 'Jharkhand'; }
  else if (pin.startsWith('560')) { city = 'Bengaluru'; state = 'Karnataka'; }
  else if (pin.startsWith('110')) { city = 'New Delhi'; state = 'Delhi NCR'; }
  else if (pin.startsWith('400')) { city = 'Mumbai'; state = 'Maharashtra'; }
  else if (pin.startsWith('700')) { city = 'Kolkata'; state = 'West Bengal'; }
  else if (pin.startsWith('600')) { city = 'Chennai'; state = 'Tamil Nadu'; }
  else if (pin.startsWith('500')) { city = 'Hyderabad'; state = 'Telangana'; }

  const availableCouriers = COURIER_PROVIDERS.filter(c => c.integrationActive).map(c => ({
    name: c.name,
    code: c.code,
