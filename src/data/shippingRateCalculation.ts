export type ShippingZone = 'Within City (Ranchi)' | 'Within State (Jharkhand)' | 'Metro to Metro' | 'Rest of India' | 'Special / North East';
 ...
 */
export function determineShippingZone(pincode: string, city: string): ShippingZone {
  ...
  if (pin.startsWith('110') || pin.startsWith('400') || pin.startsWith('560') || pin.startsWith('700') || pin.startsWith('600') || pin.startsWith('500')) {
...
export function calculatePreciseShippingCharge(params: {
  ...
  if (params.cartSubtotal >= table.freeShippingThreshold && params.shippingMode === 'Surface Freight') {
  ...
  let matchingSlab = table.slabs.find(s => weight >= s.minWeightGm && weight <= s.maxWeightGm);
  if (!matchingSlab) {
  ...
  const courierMult = matchingSlab.courierMultipliers[params.courierCode.toUpperCase()] || 1.0;
  const modeMult = params.shippingMode === 'Air Freight' && !params.hasRestrictedBatteryOrHazardous ? 1.4 : 1.0;
  const hazardousFee = params.hasRestrictedBatteryOrHazardous ? table.hazardousSurcharge : 0;

  const rawCharge = Math.round((matchingSlab.baseRateRupees * zoneMult * courierMult * modeMult) + hazardousFee);
